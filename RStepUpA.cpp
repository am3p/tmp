...
RStepUpA::RStepUpA(int kiFlag, int koFlag, double sval, double bval, double *xval, double kihval, double kohval,
	      double *Cpn,
...
{		
	...
	m_spreadHS = m_kihval + m_bval*m_spreadHRate; // up 형
	...
	*/
	...
	// ki 체크
	if(m_sval >= m_kihval)			// Up 형
		m_kiFlag = 1;

	if(m_sval <= m_kohval)    // up
		m_koFlag = 1;
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/rstepupainput.txt",ios::out);
...
double RStepUpA::getVega(int vegaN, int *vegaidx) {
...
double RStepUpA::getRho(int rhoN,int *rhoidx) {
...
double RStepUpA::cf_price()
{	
	return 7777;
}

double RStepUpA::fd_price(double *greeks)
{
	if(m_modeltype == 4) {		 
		...
		using namespace std;
			using std::ofstream;	 
			ofstream veriplot;	 
			veriplot.open("c:/logland/veriplot.txt",ios::out);	 
		...
		*/
		...
		for(i=0; i<=SM; i++) {
			knocknewV[i] = AMOUNT * (1 + m_Cpn[curtN]);  // KO rebate
			if(i<=xidx) { // up				
				newV[i] = 1; // 행사가 아래 원금
			}
			else {
				...
				if(i<hidx) // up
					newV[i] = AMOUNT * (1+m_Cpn[curtN-1]*(S[i]-xlevel)); // call 참여율 적용
				else
					newV[i] = knocknewV[i];
			}
		}
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
					...
					// no KI 영역 풀기 
					if(m_kiFlag != 1) {
						DCnewValue = DCnewValue * 1/(1+rd/(YFactor*intraTN));
						...
						// L*tempV = oldV forward로 풀기
						newV[0] = DCnewValue; // <- oldV에 복사했으므로 꼭 필요한 작업은 아님
		...
		for(i=0; i<=SM; i++) {
			if(i<=xidx) { // up
				...
				if(m_modeltype == 0 || m_modeltype == 10) 			
					knocknewV[i] = AMOUNT * MAX(MIN(1,1 + 1 - S[i]),0);	//AMOUNT * MAX(MIN(1,1 + xlevel-S[i]),0);	행사가 아님!! 1 	
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
					...
					for(i=0; i<=endSidx; i++)					
						knocknewV[i] =  AMOUNT * (1+m_Cpn[curtN-1]); //0.0; // D 0 조건
...
void RStepUpA::curttimeVol(double *vol, int day)
{
	int i;
