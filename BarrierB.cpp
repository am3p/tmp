...
BarrierB::BarrierB(int koFlag, double *sval, double *bval, double *xval, double *kohval, double *Cpn,
...
{		
	...
	*/
	...
	// ko 체크
	if(m_sval1 >= m_kohval1 || m_sval2 >= m_kohval2) // UOC 하나라도 치면 친!
		m_koFlag = 1;
...
double BarrierB::getVega(int vegaN, int *vegaidx) {
...
double BarrierB::getRho(int rhoN,int *rhoidx) {
...
double BarrierB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double BarrierB::cf_price()
{	
	return 7777;
}

double BarrierB::fd_price(double *greeks)
{
	 
//	if(m_modeltype == 0)  { // UOC 만 구현!
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				double *tempgenS;
				tempgenS = new double[m_meshSM1 + int(4*Admesh/AdmeshR)+4]; //koadd !!
			...
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					if(i>=kohidx1 || j>=kohidx2) {					
						knocknewV[i][j] = AMOUNT * (m_Cpn[curtN-1]); // rebate					
						newV[i][j] = knocknewV[i][j];
					}
					else {	
						knocknewV[i][j] = AMOUNT * m_Cpn[curtN] * MIN(S1[i]-xlevel1, S2[j]-xlevel2); // 참여율 적용
						newV[i][j] = knocknewV[i][j];
					}
				}
				else {
					knocknewV[i][j] = AMOUNT * m_Cpn[curtN] * 0; // 참여율 적용						
					newV[i][j] = knocknewV[i][j];
				}
...
void BarrierB::curttimeVol1(double *vol, int day)
{
	int i;
...
void BarrierB::curttimeVol2(double *vol, int day)
{
	int i;
...
double BarrierB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int BarrierB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
