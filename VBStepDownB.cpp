#include "VBStepDownB.h"
#include <iostream>
#include <fstream>

 

VBStepDownB::VBStepDownB(int kiFlag,  double *sval, double *bval, double *xval, double *kihval,   double *Cpn,
...
{		
	...
	m_kihval1 = new double[m_matuN];
	m_kihval2 = new double[m_matuN];
	...
	for(i=0; i<m_matuN; i++) {
		...
		m_kihval1[i] = kihval[i];
		m_kihval2[i] = kihval[m_matuN+i];
	...
	m_spreadHS1 = m_kihval1[m_matuN-1] - m_bval1*m_spreadHRate;
	m_spreadHS2 = m_kihval2[m_matuN-1] - m_bval2*m_spreadHRate;

	for(i=0; i<m_matuN; i++) { 	  
		//m_spreadXS1[i] = m_xval1[i] - m_bval1*m_spreadXRate[i];
		//m_spreadXS2[i] = m_xval2[i] - m_bval2*m_spreadXRate[i];

		m_kihval1[i] = m_kihval1[i] + m_bval1*m_shiftHRate;
		m_kihval2[i] = m_kihval2[i] + m_bval2*m_shiftHRate;
	}
...
double VBStepDownB::getVega(int vegaN, int *vegaidx) {
...
double VBStepDownB::getRho(int rhoN,int *rhoidx) {
...
double VBStepDownB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double VBStepDownB::cf_price()
{	
	return 7777;
}

double VBStepDownB::fd_price(double *greeks)
{
		...
		hlevel1 = m_kihval1[curtN-1]/m_bval1;
		startSlevel1 = m_spreadHS1/m_bval1;	 
		spreadHSlevel1 = m_spreadHS1/m_bval1;
		//spreadXSlevel1 = m_spreadXS1[curtN-1]/m_bval1;
	
		xlevel2 = m_xval2[curtN-1]/m_bval2;
		hlevel2 = m_kihval2[curtN-1]/m_bval2;
		startSlevel2 = m_spreadHS2/m_bval2;	
		spreadHSlevel2 = m_spreadHS2/m_bval2;
		//spreadXSlevel2 = m_spreadXS2[curtN-1]/m_bval2;

		 
	 
		double meshxlevel1, meshxlevel2, meshhlevel1, meshhlevel2;
		for(i=0; i<m_matuN; i++) {
			if(m_matu[i]>=0) {
				meshxlevel1 = m_xval1[i]/m_bval1;
				meshxlevel2 = m_xval2[i]/m_bval2;

				meshhlevel1 = m_kihval1[i]/m_bval1;
				meshhlevel2 = m_kihval2[i]/m_bval2;
				break;
			}
		}
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				...
				while (temps <= m_Smaxlevel1 + 2*myeps) {
					if(temps <= meshhlevel1 - Admesh + myeps)
						temph = dx;
					if(temps > meshhlevel1 - Admesh + myeps )
						temph = AdmeshR;
					if(temps > meshhlevel1 + Admesh + myeps)
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dy <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				...
				while (temps <= m_Smaxlevel2 + 2*myeps) {
					if(temps <= meshhlevel2 - Admesh + myeps)
						temph = dy;
					if(temps > meshhlevel2 - Admesh + myeps )
						temph = AdmeshR;
					if(temps > meshhlevel2 + Admesh + myeps)
		...
		*/
		...
		if(m_curtgreek == 1 && (m_matu[curtN-1] == 1 || m_matu[curtN-1] == 0)) { // 기본 그릭	Theta를 위한 +1일 저장
			for(i=0; i<outN + 4; i++) {
				for(j=0; j<outN + 4; j++) {
					sim_knockFlag = 0;
					if(SrTemp1[i] <= hlevel1*m_bval1 || SrTemp2[j] <= hlevel2*m_bval2)
						sim_knockFlag = 1;
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			if(mid_n > 0) { //
				...
				hlevel1 = m_kihval1[curtN-1]/m_bval1; // 추가
				hlevel2 = m_kihval2[curtN-1]/m_bval2; // 추가
				
				startSlevel1 = hlevel1;
				startSlevel2 = hlevel2;
		...
		if(debugFlag) {
			...
			for(i=0; i<=SM1; i++) {						
				for(j=0; j<=SM2; j++) {
					sim_knockFlag = 0;
					if((S1[i]*m_bval1 <= hlevel1*m_bval1) || (S2[j]*m_bval2 <= hlevel2*m_bval2))
						sim_knockFlag = 1;
...
void VBStepDownB::curttimeVol1(double *vol, int day)
{
	int i;
...
void VBStepDownB::curttimeVol2(double *vol, int day)
{
	int i;
...
double VBStepDownB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int VBStepDownB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
