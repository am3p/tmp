#include "StepDownBCCDB.h"
#include <iostream>
#include <fstream>

 

StepDownBCCDB::StepDownBCCDB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
{		
	...
	for(i=0; i<m_matuN; i++) {
		...
		m_bcxval1[i] = xval[m_matuN+i];
		m_bcxval2[i] = xval[3*m_matuN+i];
...
double StepDownBCCDB::getVega(int vegaN, int *vegaidx) {
...
double StepDownBCCDB::getRho(int rhoN,int *rhoidx) {
...
double StepDownBCCDB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownBCCDB::cf_price()
{	
	return 7777;
}

double StepDownBCCDB::fd_price(double *greeks)
{
	if(m_modeltype == 2) {
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				...
				if(i>=bcxidx1 && j>=bcxidx2) { //bc
					knocknewV[i][j] += AMOUNT * (m_bcCpn[curtN-1]); //(1+m_Cpn[curtN-1]); //cd
					newV[i][j] = knocknewV[i][j];				
				}
						...
						if(j<endSidx2) { // 행사 안되는 영역
							...
							if(j<bcxidx2) { 
								...
								for(i=bcxidx1; i<=SM1; i++)
									tU[i][j] += AMOUNT * (m_bcCpn[curtN-1]);  //(1+m_Cpn[curtN-1]); //cd
								for(i=endSidx1-1; i>=0; i--)
									tU[i][j] = (tU[i][j] - knockXMAu[i][j]*tU[i+1][j])/knockXMAd[i][j];
							}
						}
						...
						if(j<endSidx2) {
							...
							if(j<bcxidx2) {
								...
								for(i=endSidx1; i<=SM1; i++)
									mytempV[i][j] = AMOUNT * (m_Cpn[curtN-1]);   //(1+m_Cpn[curtN-1]); //cd
								for(i=bcxidx1; i<=SM1; i++)
									mytempV[i][j] += AMOUNT * (m_bcCpn[curtN-1]);   //(1+m_Cpn[curtN-1]); //cd
								for(i=endSidx1-1; i>=0; i--) 
									mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
							}
						}				
...
void StepDownBCCDB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownBCCDB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownBCCDB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownBCCDB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownBCCDB::fd_pricei(double *greeks)
{
