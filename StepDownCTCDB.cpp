#include "StepDownCTCDB.h"
#include <iostream>
#include <fstream>

 

StepDownCTCDB::StepDownCTCDB(int kiFlag, int koFlag, int CTFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *CThval, double *Cpn,
...
{		
	...
	m_kohval1 = kohval[0]; // ko
	m_kohval2 = kohval[1];
	m_koFlag = koFlag;
	
	m_Cpn = new double[m_matuN+2];
	m_CTCpn = new double[m_matuN+2]; // CT
	...
	m_CTCpn[m_matuN] = Cpn[m_matuN+2 + m_matuN]; // CT
	m_CTCpn[m_matuN+1] = Cpn[m_matuN+2 + m_matuN+1];

	m_CTFlag =  CTFlag;
	m_CTmatu = matu[m_matuN + 2*m_CDN];
	m_CThval1 = CThval[0];
	m_CThval2 = CThval[1];
	...
	*/
	...
	if(m_CTmatu >=0 && m_sval1 <= m_CThval1 || m_sval2 <= m_CThval2)
		m_CTFlag = 1;
...
double StepDownCTCDB::getVega(int vegaN, int *vegaidx) {
...
double StepDownCTCDB::getRho(int rhoN,int *rhoidx) {
...
double StepDownCTCDB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownCTCDB::cf_price()
{	
	return 7777;
}

double StepDownCTCDB::fd_price(double *greeks)
{
	if(m_CTmatu >= 0 && m_CTFlag == 0)  { 
		...
		double DCValue, CDlegDCValue, CTDCValue;	
		...
		double **CToldV, **CTnewV; //CT
		double **CTknockoldV, **CTknocknewV;
		...
		double **NCTXMAu, **NCTXMAd, **NCTXMAl; //CT
		double **NCTYMAu, **NCTYMAd, **NCTYMAl;
		...
		double CThlevel1, CThlevel2;
		//double CTstartSlevel1, CTstartSlevel2;
		int CTstartSidx1, CTstartSidx2;
		int CThidx1, CThidx2;
		...
		CThlevel1 = m_CThval1/m_bval1;
		CThlevel2 = m_CThval2/m_bval2;
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				...
				while (temps <= m_Smaxlevel1 + 2*myeps) {
					...
					if(temps > CThlevel1 - Admesh + myeps ) //CT
						temph = AdmeshR;
					if(temps > CThlevel1 + Admesh + myeps)
						temph = dx;
		...
		CThidx1 = findinteridx(CThlevel1, S1, SM1); //CT
		CTstartSidx1 = CThidx1;
	 	CThidx2 = findinteridx(CThlevel2, S2, SM2);
		CTstartSidx2 = CThidx2;
		...
		CToldV = (double **)malloc((size_t)(SM1+1)*sizeof(double *)); //CT
		CTnewV = (double **)malloc((size_t)(SM1+1)*sizeof(double *));		
		
		CTknockoldV = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		CTknocknewV = (double **)malloc((size_t)(SM1+1)*sizeof(double *));	
		
		NCTXMAl = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		NCTXMAd = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		NCTXMAu = (double **)malloc((size_t)(SM1+1)*sizeof(double *));

		NCTYMAl = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		NCTYMAd = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		NCTYMAu = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		...
		for(i=0; i<SM1+1; i++) {
			...
			CToldV[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double)); //CT	
			CTnewV[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));	
				
			CTknockoldV[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));	
			CTknocknewV[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));	
			
			NCTXMAl[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));
			NCTXMAd[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));
			NCTXMAu[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));

			NCTYMAl[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));
			NCTYMAd[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));
			NCTYMAu[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					CTknocknewV[i][j] = AMOUNT * (1+m_CTCpn[curtN-1]); //CT
					CTnewV[i][j] = CTknocknewV[i][j];
				}
				else {
					...
					if(m_modeltype == 0 || m_modeltype == 10) { // 원금비보장 stepdown
						...
						CTknocknewV[i][j] = AMOUNT *  (-1 + MIN(MIN(S1[i],S2[j]),1)); //MIN(S1[i],S2[j]);	//cd    AMOUNT *  (-1 + MIN(S1[i],S2[j]));  //CT

					}

					if(m_modeltype == 1 || m_modeltype == 11) { // 원금보장 KI 있는 stepdown
						knocknewV[i][j] = AMOUNT * (m_Cpn[curtN+1]);  //(1+m_Cpn[curtN+1]); //cd
						CTknocknewV[i][j] = AMOUNT * (m_CTCpn[curtN+1]);  //(1+m_Cpn[curtN+1]); //cd  //CT
					}
					
					if(m_modeltype == 3) { //CT(X) 원금보장, CT(O) 원금비보장
						knocknewV[i][j] = AMOUNT * (m_Cpn[curtN+1]);
						CTknocknewV[i][j] = AMOUNT *  (-1 + MIN(MIN(S1[i],S2[j]),1));
					}
					//모형별 만기 payoff end
					...
					if(i>hidx1 && j>hidx2) {
						...
						CTnewV[i][j] = AMOUNT * (m_CTCpn[curtN]);  //(1+m_Cpn[curtN]); // dummy cpn 저장 //CT
					}
					else {
						newV[i][j] = knocknewV[i][j];
						CTnewV[i][j] = CTknocknewV[i][j];  //CT
					}
				}
			...
			for(i=spreadHSidx1; i<=SM1; i++) {
				for(j=spreadHSidx2; j<=SM2; j++) {
					if(i<=hidx1 || j<=hidx2) {
						...
						spreadvalue = AMOUNT * (1+m_CTCpn[curtN])/(spmax-spmin) * (MIN(S1[i],S2[j])-spmin);
						//newV[i][j] = MAX(newV[i][j],spreadvalue); //cd
						CTnewV[i][j] = MAX(CTnewV[i][j],spreadvalue-AMOUNT);
					}
				}
			}
		} // H spread
		*/
			...
			for(i=0; i<=SM1; i++) {
				for(j=0; j<=SM2; j++) {
					if(i<=xidx1 || j<=xidx2) {
						...
						spreadvalue = AMOUNT * ( m_spreadXRate[curtN-1] * (MIN(S1[i],S2[j])-spmax) + 1+m_CTCpn[curtN-1]);
						
						//knocknewV[i][j] = MAX(knocknewV[i][j], spreadvalue);  //cd
						//newV[i][j] = MAX(newV[i][j],spreadvalue);  //cd
						CTknocknewV[i][j] = MAX(CTknocknewV[i][j], spreadvalue-AMOUNT);
						CTnewV[i][j] = MAX(CTnewV[i][j],spreadvalue-AMOUNT);
		...
		if(m_kiFlag == 1) {	// ki or ko 되었으면..
			...
			for(i=0; i<=SM1; i++)
				memcpy(CTnewV[i],CTknocknewV[i],(size_t)((SM2+1)*sizeof(double)));
			/*
			for(i=0; i<=SM1; i++) 
				for(j=0; j<=SM2; j++)
					newV[i][j] = knocknewV[i][j];
		...
		if(m_curtgreek == 1 && (m_matu[curtN-1] == 1 || m_matu[curtN-1] == 0)) { // 기본 그릭	Theta를 위한 +1일 저장
			for(i=0; i<outN + 4; i++) {
				for(j=0; j<outN + 4; j++) {
					...
					sim_CTFlag = 0;
					if(1<=m_CTmatu && (SrTemp1[i] <= m_CThval1 || SrTemp2[j] <= m_CThval2))
						sim_CTFlag = 1;
					...
					if(sim_CTFlag == 1) {
						...
						if(sim_knockFlag == 1) {
							SrV1Temp[i][j] = interp2(sleveltemp1, sleveltemp2, S1, S2, CTknocknewV, SM1, SM2);  
							SrV1Temp[i][j] -= interp2(sleveltemp1, sleveltemp2, S1, S2, CDnewV, SM1, SM2); // cd
						}
						else {			
							SrV1Temp[i][j] = interp2(sleveltemp1, sleveltemp2, S1, S2, CTnewV, SM1, SM2);
							SrV1Temp[i][j] -= interp2(sleveltemp1, sleveltemp2, S1, S2, CDnewV, SM1, SM2); // cd
						}
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 
				...
				for(i=1; i<SM1; i++) { //x축 풀기 위하여 행렬 만들기
					...
					if(matuLoop <= m_CTmatu) {
						NCTXMAl[i][j] = knockXMAl[i][j];				
						NCTXMAd[i][j] = knockXMAd[i][j];					
						NCTXMAu[i][j] = knockXMAu[i][j];
					}
				...
				if(matuLoop <= m_CTmatu) { //CT
					NCTXMAl[SM1-1][j] = knockXMAl[SM1-1][j];			
					NCTXMAd[SM1-1][j] = knockXMAd[SM1-1][j];

					NCTXMAl[CTstartSidx1][j] = 0;		
					NCTXMAd[CTstartSidx1][j] = 1;			
					NCTXMAu[CTstartSidx1][j] = 0;	
						
					//LU decomposition			
					for(i=startSidx1+1; i<=SM1-1; i++) {				
						NCTXMAl[i][j] = NCTXMAl[i][j]/NCTXMAd[i-1][j];				
						NCTXMAd[i][j] = NCTXMAd[i][j] - NCTXMAl[i][j]*NCTXMAu[i-1][j];			
					}			
				}
			...
			for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 
				...
				for(j=1; j<SM2; j++) { //x축 풀기 위하여 행렬 만들기
					...
					if(matuLoop <= m_CTmatu) {							
						NCTYMAl[i][j] = knockYMAl[i][j];				
						NCTYMAd[i][j] = knockYMAd[i][j];					
						NCTYMAu[i][j] = knockYMAu[i][j];
					}
				...
				if(m_kiFlag != 1) {
 				...
 				if(matuLoop <= m_CTmatu) {
					NCTYMAl[i][SM2-1] = knockYMAl[i][SM2-1];			
					NCTYMAd[i][SM2-1] = knockYMAd[i][SM2-1];

					NCTYMAl[i][CTstartSidx2] = 0;		
					NCTYMAd[i][CTstartSidx2] = 1;			
					NCTYMAu[i][CTstartSidx2] = 0;	
						
					//LU decomposition			
					for(j=startSidx2+1; j<=SM2-1; j++) {				
						NCTYMAl[i][j] = NCTYMAl[i][j]/NCTYMAd[i][j-1];				
						NCTYMAd[i][j] = NCTYMAd[i][j] - NCTYMAl[i][j]*NCTYMAu[i][j-1];			
					}			
				}
			...
			for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
				CTDCValue = CTDCValue * 1/(1+rd/(YFactor*intraTN));
				...
				for(i=0; i<=SM1; i++)			
					memcpy(CTknockoldV[i],CTknocknewV[i],(size_t)((SM2+1)*sizeof(double)));
			
				if(m_kiFlag != 1)
					for(i=0; i<=SM1; i++)				
						memcpy(CToldV[i],CTnewV[i],(size_t)((SM2+1)*sizeof(double)));
				/*
				for(i=0; i<=SM1; i++)
					for(j=0; j<=SM2; j++) {
				...
				*/
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
					for(i=0; i<=SM1; i++) 
						tU[i][0] = CTDCValue; //0.0;

					for(j=1; j<SM2; j++) { //y축 고정 풀기

						for(i=1; i<SM1; i++) 
							vecV1[i] = CTknockoldV[i][j] + 2*coeV1[i][j]*(-CTknockoldV[i+1][j-1]+CTknockoldV[i-1][j-1] +CTknockoldV[i+1][j+1]-CTknockoldV[i-1][j+1]); // herexxx

						tU[0][j] = CTDCValue; //0.0;
						for(i=1; i<SM1; i++)
					...
					//////////////////////////////////////////////////////// newV 구하기!!
					for(j=0; j<=SM2; j++)
						CTknocknewV[0][j]=CTDCValue; //0.0;
		 
					for(i=1; i<SM1; i++) {  //x축 고정시켜서.. 				 

						for(j=1; j<SM2; j++) 				
							vecV2[j] =tU[i][j] + 0*coeV2[i][j] *(-tU[i+1][j-1]+tU[i-1][j-1] +tU[i+1][j+1]-tU[i-1][j+1]);//(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);		//herexxx		   				 		 		

						CTknocknewV[i][0] = CTDCValue; //0.0;
						for(j=1; j<SM2; j++) 
							CTknocknewV[i][j] = vecV2[j] - knockYMAl[i][j]*CTknocknewV[i][j-1];					
						CTknocknewV[i][SM2-1] = CTknocknewV[i][SM2-1]/knockYMAd[i][SM2-1];
						for(j=SM2-2; j>=0; j--) 
							CTknocknewV[i][j] = (CTknocknewV[i][j] - knockYMAu[i][j]*CTknocknewV[i][j+1])/knockYMAd[i][j];					
				
						if(SmeshType2 == 0)				
							CTknocknewV[i][SM2] = 2*CTknocknewV[i][SM2-1] - CTknocknewV[i][SM2-2];
						else
							CTknocknewV[i][SM2] = (h2[SM2-1]+h2[SM2-2])/h2[SM2-2] * CTknocknewV[i][SM2-1] - h2[SM2-1]/h2[SM2-2] * CTknocknewV[i][SM2-2];
				
					} // for(i x축.. 고정 다 품

					for(j=0; j<=SM2; j++) {
						if(SmeshType1 == 0) 
							CTknocknewV[SM1][j] = 2*CTknocknewV[SM1-1][j] - CTknocknewV[SM1-2][j];
						else
							CTknocknewV[SM1][j] = (h1[SM1-1]+h1[SM1-2])/h1[SM1-2] * CTknocknewV[SM1-1][j] - h1[SM1-1]/h1[SM1-2] * CTknocknewV[SM1-2][j];
					} // knocknewV 완결
					...
					if(m_kiFlag != 1) {				
						...
						for(i=startSidx1; i<=SM1; i++)
							tU[i][startSidx2] = CTknocknewV[i][startSidx2];

						for(j=startSidx2+1; j<SM2; j++) { //y축 고정 풀기						
							for(i=startSidx1+1; i<SM1; i++) 
								vecV1[i] = CToldV[i][j] + 2*coeV1[i][j]*(-CToldV[i+1][j-1]+CToldV[i-1][j-1] +CToldV[i+1][j+1]-CToldV[i-1][j+1]) ; //herexxx
                        
							tU[startSidx1][j] = CTknocknewV[startSidx1][j];
							for(i=startSidx1+1; i<SM1; i++)
						...
						for(j=startSidx2; j<=SM2; j++)
							CTnewV[startSidx1][j]=CTknocknewV[startSidx1][j]; // D.C 적용
						...
						for(i=startSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
							...
							CTnewV[i][startSidx2] = CTknocknewV[i][startSidx2];
							for(j=startSidx2+1; j<SM2; j++) 
								CTnewV[i][j] = vecV2[j] - YMAl[i][j]*CTnewV[i][j-1];					
							CTnewV[i][SM2-1] = CTnewV[i][SM2-1]/YMAd[i][SM2-1];
							for(j=SM2-2; j>=startSidx2; j--) 
								CTnewV[i][j] = (CTnewV[i][j] - YMAu[i][j]*CTnewV[i][j+1])/YMAd[i][j];					
				
							if(SmeshType2 == 0)				
								CTnewV[i][SM2] = 2*CTnewV[i][SM2-1] - CTnewV[i][SM2-2];
							else
								CTnewV[i][SM2] = (h2[SM2-1]+h2[SM2-2])/h2[SM2-2] * CTnewV[i][SM2-1] - h2[SM2-1]/h2[SM2-2] * CTnewV[i][SM2-2];
				
						} // for(j y축.. 고정 다 품

						for(j=startSidx2; j<=SM2; j++) {
							if(SmeshType1 == 0) 
								CTnewV[SM1][j] = 2*CTnewV[SM1-1][j] - CTnewV[SM1-2][j];
							else
								CTnewV[SM1][j] = (h1[SM1-1]+h1[SM1-2])/h1[SM1-2] * CTnewV[SM1-1][j] - h1[SM1-1]/h1[SM1-2] * CTnewV[SM1-2][j];
						} // newV 완결
					...
					for(j=1; j<SM2; j++) { //y축 고정 풀기
						...
						if(j<endSidx2) { // 행사 안되는 영역  if(j<endSidx2)
							...
							for(i=endSidx1; i<=SM1; i++)
								tU[i][j] = AMOUNT * (m_CTCpn[curtN-1]); //(1+m_CTCpn[curtN-1]);
					...
					//////////////////////////////////////////////////////// newV 구하기!!
					for(j=0; j<=SM2; j++)
						CTknocknewV[0][j]=CTDCValue; //0.0; // D.C 적용
					...
					for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 				 
						if(i<endSidx1) {
							...
							for(j=endSidx2; j<=SM2; j++)
								CTknocknewV[i][j] = AMOUNT * (m_CTCpn[curtN-1]);  //(1+m_CTCpn[curtN-1]);
					...
					for(i=1; i<SM1; i++) { //y축 고정 풀기
						...
						if(i<endSidx1) { // 행사 안되는 영역  if(j<endSidx2)
							for(j=1; j<SM2; j++) 
								vecV2[j] = CTknockoldV[i][j] + 2*coeV2[i][j]*(-CTknockoldV[i+1][j-1]+CTknockoldV[i-1][j-1] + CTknockoldV[i+1][j+1] - CTknockoldV[i-1][j+1]) ; //herexxx
					...
					//////////////////////////////////////////////////////// newV 구하기!!
					for(i=0; i<=SM1; i++)
						mytempV[i][0]=CTDCValue; //0.0; // D.C 적용
					...
					for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 				 
						if(j<endSidx2) {
							...
							for(i=1; i<endSidx1; i++)
								mytempV[i][j] = vecV1[i] - knockXMAl[i][j]*mytempV[i-1][j];
							for(i=endSidx1; i<=SM1; i++)
								mytempV[i][j] = AMOUNT * (m_CTCpn[curtN-1]); //(1+m_CTCpn[curtN-1]);
							for(i=endSidx1-1; i>=0; i--) 
								mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
						}				
					} // for(j y축.. 고정 다 품
					...
					for(i=0; i<=SM1; i++) 
						for(j=0; j<=SM2; j++)
							CTknocknewV[i][j] = (mytempV[i][j] + CTknocknewV[i][j])/2.0;
					...
					if(m_spreadXRate[curtN-1] > 0){// && m_Cpn[curtN-1]>0) { // X spread는 만기와 조기상환일에 가능
						...
						spvalueR =  AMOUNT * (m_CTCpn[curtN-1]);
						for(i=xidx1; i<=SM1; i++) {
							maxindexR = 0;
							maxvalueR = CTknocknewV[i][0];
							for(j=1; j<=xidx2; j++) {
								if(maxvalueR<CTknocknewV[i][j]) {
									maxindexR = j;
									maxvalueR = CTknocknewV[i][j];
								}
							}
							if(maxindexR < xidx2) {
								...
								for(j=maxindexR+1; j<tempindex; j++) {
									...
									CTknocknewV[i][j] = MAX(CTknocknewV[i][j],spreadvalue);
								}
							}
						}
					...
					if(m_kiFlag != 1) {		
						...
						for(j=startSidx2+1; j<SM2; j++) { //y축 고정 풀기
							if(j<endSidx2) {
								for(i=startSidx1+1; i<SM1; i++) 
									vecV1[i] = CToldV[i][j] + 2*coeV1[i][j]*(-CToldV[i+1][j-1]+CToldV[i-1][j-1] + CToldV[i+1][j+1] -CToldV[i-1][j+1]);//-2*0.5*rd*dt*CToldV[i][j]; //herexxx
								...
								for(i=startSidx1+1; i<endSidx1; i++) // 행사 안되는 영역 
									vecV1[i] = CToldV[i][j] + 2*coeV1[i][j]*(-CToldV[i+1][j-1]+CToldV[i-1][j-1] + CToldV[i+1][j+1] - CToldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
						...
						for(i=startSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
							if(i<endSidx1) {
								for(j=startSidx2+1; j<SM2; j++) 				
									vecV2[j] =tU[i][j] + 0*coeV2[i][j] *(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);//-0*0.5*rd*dt*tU[i][j];  //herexxx					   				 		 		
								...
								for(j=startSidx2+1; j<endSidx2; j++) 
									CTnewV[i][j] = vecV2[j] - YMAl[i][j]*CTnewV[i][j-1];					
								for(j=endSidx2; j<=SM2; j++)
									CTnewV[i][j] = AMOUNT * (m_CTCpn[curtN-1]); //(1+m_CTCpn[curtN-1]);
						...
						for(j=startSidx2; j<=SM2; j++)
							tU[startSidx1][j] = CTknocknewV[startSidx2][j];

						for(i=startSidx1+1; i<SM1; i++) { //y축 고정 풀기
							if(i<endSidx1) {
								for(j=startSidx2+1; j<SM2; j++) 
									vecV2[j] = CToldV[i][j] + 2*coeV2[i][j]*(-CToldV[i+1][j-1]+CToldV[i-1][j-1] + CToldV[i+1][j+1] - CToldV[i-1][j+1]);//-2*0.5*rd*dt*CToldV[i][j]; //herexxx
						...
						//////////////////////////////////////////////////////// newV 구하기!!
						for(i=startSidx1; i<=SM1; i++)
							mytempV[i][startSidx2]=CTknocknewV[i][startSidx2]; // D.C 적용
		 
						for(j=startSidx2+1; j<SM2; j++) {  //y축 고정시켜서.. 				 
							if(j<endSidx2) {
								for(i=startSidx1+1; i<SM1; i++) 				
									vecV1[i] =tU[i][j] + 0*coeV1[i][j] *(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);//-0*0.5*rd*dt*tU[i][j];  //herexxx					   				 		 		

								mytempV[startSidx1][j] = CTknocknewV[startSidx1][j];
								for(i=startSidx1+1; i<SM1; i++) 
						...
						for(i=0; i<=SM1; i++) {
							for(j=0; j<=SM2; j++) {
								if(i<=startSidx1 || j<=startSidx2)
									mytempV[i][j] = CTknocknewV[i][j];
							}
						}

						for(i=0; i<=SM1; i++) 
							for(j=0; j<=SM2; j++)
								CTnewV[i][j] = (CTnewV[i][j] + mytempV[i][j])/2.0;

						// 평균으로 구하기 e
		
						if(m_spreadXRate[curtN-1] > 0){// && m_CTCpn[curtN-1]>0) { // X spread는 만기와 조기상환일에 가능
							double spreadvalue;
							double   spmax;
						...
						spreadvalue = AMOUNT * ( m_spreadXRate[curtN-1] * (MIN(S1[i],S2[j])-spmax) + 1+m_CTCpn[curtN-1]);
							...
							for(i=xidx1; i<=SM1; i++) {
								maxindexR = 0;
								maxvalueR = CTnewV[i][0];
								for(j=1; j<=xidx2; j++) {
									if(maxvalueR<CTnewV[i][j]) {
										maxindexR = j;
										maxvalueR = CTnewV[i][j];
									}
								}
								if(maxindexR < xidx2) {
									tempindex = 2*xidx2 - spreadXSidx2;
									for(j=maxindexR+1; j<tempindex; j++) {
										spreadvalue = (maxvalueR-spvalueR)/(S2[maxindexR]-S2[tempindex]) * (S2[j]-S2[tempindex]) + spvalueR;
										CTnewV[i][j] = MAX(CTnewV[i][j],spreadvalue);
									}
								}
							}
				...
				else {  //////////////////////////////////////////////if(matuLoop > m_CTmatu)
					//////////////////////////////////////////55555555555555555555555555555555555555555555555555555555555555555555555555555555555555555

					if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
					 
							for(i=CTstartSidx1; i<=SM1; i++)
								tU[i][CTstartSidx2] = CTknocknewV[i][CTstartSidx2];

							for(j=CTstartSidx2+1; j<SM2; j++) { //y축 고정 풀기						
								for(i=CTstartSidx1+1; i<SM1; i++) 
									vecV1[i] = knockoldV[i][j] + 2*coeV1[i][j]*(-knockoldV[i+1][j-1]+knockoldV[i-1][j-1] +knockoldV[i+1][j+1]-knockoldV[i-1][j+1]) ; //herexxx
                        
								tU[CTstartSidx1][j] = CTknocknewV[CTstartSidx1][j];
								for(i=CTstartSidx1+1; i<SM1; i++)
									tU[i][j] = vecV1[i] - NCTXMAl[i][j]*tU[i-1][j];
								tU[SM1-1][j] = tU[SM1-1][j]/NCTXMAd[SM1-1][j];
								for(i=SM1-2; i>=CTstartSidx1; i--) 				
									tU[i][j] = (tU[i][j] - NCTXMAu[i][j]*tU[i+1][j])/NCTXMAd[i][j];
							...
							//////////////////////////////////////////////////////// newV 구하기!!
							for(j=CTstartSidx2; j<=SM2; j++)
								knocknewV[CTstartSidx1][j]=CTknocknewV[CTstartSidx1][j]; // D.C 적용
		 
							for(i=CTstartSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 

								for(j=CTstartSidx2+1; j<SM2; j++) 				
									vecV2[j] =tU[i][j] + 0*coeV2[i][j] *(-tU[i+1][j-1]+tU[i-1][j-1] +tU[i+1][j+1]-tU[i-1][j+1]) ;//(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);	//herexxx				   				 		 		

								knocknewV[i][CTstartSidx2] = CTknocknewV[i][CTstartSidx2];
								for(j=CTstartSidx2+1; j<SM2; j++) 
									knocknewV[i][j] = vecV2[j] - NCTYMAl[i][j]*knocknewV[i][j-1];					
								knocknewV[i][SM2-1] = knocknewV[i][SM2-1]/NCTYMAd[i][SM2-1];
								for(j=SM2-2; j>=CTstartSidx2; j--) 
									knocknewV[i][j] = (knocknewV[i][j] - NCTYMAu[i][j]*knocknewV[i][j+1])/NCTYMAd[i][j];					
							...
							for(i=0; i<=SM1; i++)
								for(j=0; j<=SM2; j++) {
									if(i<=CTstartSidx1 || j<=CTstartSidx2)
										knocknewV[i][j] = CTknocknewV[i][j];
								}
						// no KI 영역 풀기 
						if(m_kiFlag != 1) {				
					 
							for(i=CTstartSidx1; i<=SM1; i++)
								tU[i][CTstartSidx2] = CTnewV[i][CTstartSidx2];

							for(j=CTstartSidx2+1; j<SM2; j++) { //y축 고정 풀기						
								for(i=CTstartSidx1+1; i<SM1; i++) 
									vecV1[i] = oldV[i][j] + 2*coeV1[i][j]*(-oldV[i+1][j-1]+oldV[i-1][j-1] +oldV[i+1][j+1]-oldV[i-1][j+1]) ; //herexxx
                        
								tU[CTstartSidx1][j] = CTnewV[CTstartSidx1][j];
								for(i=CTstartSidx1+1; i<SM1; i++)
									tU[i][j] = vecV1[i] - NCTXMAl[i][j]*tU[i-1][j];
								tU[SM1-1][j] = tU[SM1-1][j]/NCTXMAd[SM1-1][j];
								for(i=SM1-2; i>=CTstartSidx1; i--) 				
							...
							//////////////////////////////////////////////////////// newV 구하기!!
							for(j=CTstartSidx2; j<=SM2; j++)
								newV[CTstartSidx1][j]=CTnewV[CTstartSidx1][j]; // D.C 적용
							...
							for(i=CTstartSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
								...
								newV[i][CTstartSidx2] = CTnewV[i][CTstartSidx2];
								for(j=CTstartSidx2+1; j<SM2; j++) 
									newV[i][j] = vecV2[j] - NCTYMAl[i][j]*newV[i][j-1];					
								newV[i][SM2-1] = newV[i][SM2-1]/NCTYMAd[i][SM2-1];
								for(j=SM2-2; j>=CTstartSidx2; j--) 
									newV[i][j] = (newV[i][j] - NCTYMAu[i][j]*newV[i][j+1])/NCTYMAd[i][j];					
							...
							for(j=CTstartSidx2+1; j<SM2; j++) { //y축 고정 풀기
								if(j<endSidx2) {
									for(i=CTstartSidx1+1; i<SM1; i++) 
										vecV1[i] = knockoldV[i][j] + 2*coeV1[i][j]*(-knockoldV[i+1][j-1]+knockoldV[i-1][j-1] + knockoldV[i+1][j+1] - knockoldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
									...
									for(i=CTstartSidx1+1; i<endSidx1; i++) // 행사 안되는 영역 
										vecV1[i] = knockoldV[i][j] + 2*coeV1[i][j]*(-knockoldV[i+1][j-1]+knockoldV[i-1][j-1] + knockoldV[i+1][j+1] - knockoldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
							
									tU[CTstartSidx1][j] = CTknocknewV[CTstartSidx1][j];
									for(i=CTstartSidx1+1; i<endSidx1; i++) 
										tU[i][j] = vecV1[i] - NCTXMAl[i][j]*tU[i-1][j];
									for(i=endSidx1; i<=SM1; i++)
										tU[i][j] = AMOUNT * (m_Cpn[curtN-1]); // (1+m_Cpn[curtN-1]);
									for(i=endSidx1-1; i>=CTstartSidx1; i--)
										tU[i][j] = (tU[i][j] - NCTXMAu[i][j]*tU[i+1][j])/NCTXMAd[i][j];
								}
							}// for(j=1
							...
							for(i=CTstartSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
								if(i<endSidx1) {
									...
									for(j=CTstartSidx2+1; j<endSidx2; j++) 				
										vecV2[j] =tU[i][j] + 0*coeV2[i][j] *(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);//-0*0.5*rd*dt*tU[i][j];  //herexxx					   				 		 		
									...
									for(j=CTstartSidx2+1; j<endSidx2; j++) 
										knocknewV[i][j] = vecV2[j] - NCTYMAl[i][j]*knocknewV[i][j-1];					
									for(j=endSidx2; j<=SM2; j++)
										knocknewV[i][j] = AMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]);
									for(j=endSidx2-1; j>=CTstartSidx2; j--) 
										knocknewV[i][j] = (knocknewV[i][j] - NCTYMAu[i][j]*knocknewV[i][j+1])/NCTYMAd[i][j];					
								}				
							} // for(j y축.. 고정 다 품
							...
							for(j=CTstartSidx2; j<=SM2; j++)
								tU[CTstartSidx1][j] = CTknocknewV[CTstartSidx2][j];

							for(i=CTstartSidx1+1; i<SM1; i++) { //y축 고정 풀기
								if(i<endSidx1) {
									for(j=CTstartSidx2+1; j<SM2; j++) 
										vecV2[j] = knockoldV[i][j] + 2*coeV2[i][j]*(-knockoldV[i+1][j-1]+knockoldV[i-1][j-1] + knockoldV[i+1][j+1] - knockoldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
							
									tU[i][CTstartSidx2] = CTknocknewV[i][CTstartSidx2];
									for(j=CTstartSidx2+1; j<SM2; j++)
										tU[i][j] = vecV2[j] - NCTYMAl[i][j]*tU[i][j-1];
									tU[i][SM2-1] = tU[i][SM2-1]/NCTYMAd[i][SM2-1];
									for(j=SM2-2; j>=CTstartSidx2; j--) 				
										tU[i][j] = (tU[i][j] - NCTYMAu[i][j]*tU[i][j+1])/NCTYMAd[i][j];
									...
									for(j=CTstartSidx2+1; j<endSidx2; j++) // 행사 안되는 영역 
										vecV2[j] = knockoldV[i][j] + 2*coeV2[i][j]*(-knockoldV[i+1][j-1]+knockoldV[i-1][j-1] + knockoldV[i+1][j+1] - knockoldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
							...
							//////////////////////////////////////////////////////// newV 구하기!!
							for(i=CTstartSidx1; i<=SM1; i++)
								mytempV[i][CTstartSidx2]=CTknocknewV[i][CTstartSidx2]; // D.C 적용
		 
							for(j=CTstartSidx2+1; j<SM2; j++) {  //y축 고정시켜서.. 				 
								if(j<endSidx2) {
									for(i=CTstartSidx1+1; i<SM1; i++) 				
										vecV1[i] =tU[i][j] + 0*coeV1[i][j] *(-tU[i-1][j+1]+tU[i-1][j-1] + tU[i+1][j+1] - tU[i+1][j-1]);//-0*0.5*rd*dt*tU[i][j];  //herexxx					   				 		 		

									mytempV[CTstartSidx1][j] = CTknocknewV[CTstartSidx1][j];
									for(i=CTstartSidx1+1; i<SM1; i++) 
										mytempV[i][j] = vecV1[i] - NCTXMAl[i][j]*mytempV[i-1][j];					
									mytempV[SM1-1][j] = mytempV[SM1-1][j]/NCTXMAd[SM1-1][j];
									for(i=SM1-2; i>=CTstartSidx1; i--) 
										mytempV[i][j] = (mytempV[i][j] - NCTXMAu[i][j]*mytempV[i+1][j])/NCTXMAd[i][j];					
									...
									for(i=CTstartSidx1+1; i<endSidx1; i++) 
										mytempV[i][j] = vecV1[i] - NCTXMAl[i][j]*mytempV[i-1][j];					
									for(i=endSidx1; i<=SM1; i++)
										mytempV[i][j] = AMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]);
									for(i=endSidx1-1; i>=CTstartSidx1; i--) 
										mytempV[i][j] = (mytempV[i][j] - NCTXMAu[i][j]*mytempV[i+1][j])/NCTXMAd[i][j];					
								}				
							} // for(j y축.. 고정 다 품
						...
						*/
						...
						if(m_kiFlag != 1) {		
							...
							for(j=CTstartSidx2+1; j<SM2; j++) { //y축 고정 풀기
								if(j<endSidx2) {
									for(i=CTstartSidx1+1; i<SM1; i++) 
										vecV1[i] = oldV[i][j] + 2*coeV1[i][j]*(-oldV[i+1][j-1]+oldV[i-1][j-1] + oldV[i+1][j+1] - oldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
							...
							for(i=CTstartSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
								if(i<endSidx1) {
									...
									for(j=CTstartSidx2+1; j<endSidx2; j++) 
										newV[i][j] = vecV2[j] - NCTYMAl[i][j]*newV[i][j-1];					
									for(j=endSidx2; j<=SM2; j++)
										newV[i][j] = AMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]);
									for(j=endSidx2-1; j>=CTstartSidx2; j--) 
										newV[i][j] = (newV[i][j] - NCTYMAu[i][j]*newV[i][j+1])/NCTYMAd[i][j];					
								}				
							} // for(j y축.. 고정 다 품
							...
							for(j=CTstartSidx2; j<=SM2; j++)
								tU[CTstartSidx1][j] = CTnewV[CTstartSidx2][j];

							for(i=CTstartSidx1+1; i<SM1; i++) { //y축 고정 풀기
								if(i<endSidx1) {
									for(j=CTstartSidx2+1; j<SM2; j++) 
										vecV2[j] = oldV[i][j] + 2*coeV2[i][j]*(-oldV[i+1][j-1]+oldV[i-1][j-1] + oldV[i+1][j+1] - oldV[i-1][j+1]);//-2*0.5*rd*dt*oldV[i][j]; //herexxx
							...
							//////////////////////////////////////////////////////// newV 구하기!!
							for(i=CTstartSidx1; i<=SM1; i++)
								mytempV[i][CTstartSidx2]=CTnewV[i][CTstartSidx2]; // D.C 적용
							...
							for(j=CTstartSidx2+1; j<SM2; j++) {  //y축 고정시켜서.. 				 
								if(j<endSidx2) {
									...
									mytempV[CTstartSidx1][j] = CTnewV[CTstartSidx1][j];
									for(i=CTstartSidx1+1; i<SM1; i++) 
										mytempV[i][j] = vecV1[i] - NCTXMAl[i][j]*mytempV[i-1][j];					
									mytempV[SM1-1][j] = mytempV[SM1-1][j]/NCTXMAd[SM1-1][j];
									for(i=SM1-2; i>=CTstartSidx1; i--) 
				...
				*/
				...
				for(i=0; i<=SM1; i++) {
					...
					for(j=0; j<=SM2; j++) {
						...
						CTknocknewV[i][j] = interp2(S1[i]-alpha1*disc_div1/m_bval1,S2[j]-alpha2*disc_div2/m_bval2,S1,S2,CTknockoldV,SM1,SM2); // SM1은.. S1 분할 수.. 0부터 시작.. 마지막 인덱스
					...
					*/
					...
					for(i=0; i<=SM1; i++) {
						...
						for(j=0; j<=SM2; j++) {
							...
							CTnewV[i][j] = interp2(S1[i]-alpha1*disc_div1/m_bval1,S2[j]-alpha2*disc_div2/m_bval2,S1,S2,CToldV,SM1,SM2); // SM1은.. S1 분할 수.. 0부터 시작.. 마지막 인덱스
			...
			if(m_curtgreek == 1 && matuLoop == 1) { // 기본 그릭	Theta를 위한 +1일 저장
				for(i=0; i<outN + 4; i++) {
					for(j=0; j<outN + 4; j++) {
						...
						sim_CTFlag = 0;
						if(matuLoop <= m_CTmatu && (SrTemp1[i] <= m_CThval1 || SrTemp2[j] <= m_CThval2))						
							sim_CTFlag = 1;
		...
		if(debugFlag) {
			...
			for(i=0; i<=SM1; i++) {						
				for(j=0; j<=SM2; j++) {
					...
					sim_CTFlag = 0;
					if((S1[i]*m_bval1 <= m_CThval1) || (S2[j]*m_bval2 <= m_CThval2))
						sim_CTFlag = 1;
					...
					if(sim_CTFlag == 1) {
						if(sim_knockFlag == 1) {				
							valuetemp = interp2(stemp1/m_bval1, stemp2/m_bval2, S1, S2, CTknocknewV, SM1, SM2);
							valuetemp -= interp2(stemp1/m_bval1, stemp2/m_bval2, S1, S2, CDnewV, SM1, SM2);
						}
						else {
							valuetemp = interp2(stemp1/m_bval1, stemp2/m_bval2, S1, S2, CTnewV, SM1, SM2);
							valuetemp -= interp2(stemp1/m_bval1, stemp2/m_bval2, S1, S2, CDnewV, SM1, SM2);
						}
					}
					else {
		...
		if(m_curtgreek == 1 || m_curtgreek == 2) {
			for(i=0; i<outN + 4; i++) {			
				for(j=0; j<outN + 4; j++) {
					...
					sim_CTFlag = 0;
					if(SrTemp1[i] <= m_CThval1 || SrTemp2[j] <= m_CThval2)
						sim_CTFlag = 1;
					...
					if(sim_CTFlag == 1) {
						if(sim_knockFlag == 1) {			
							SrVTemp[i][j] = interp2(sleveltemp1, sleveltemp2, S1, S2, CTknocknewV, SM1, SM2);
							SrVTemp[i][j] -= interp2(sleveltemp1, sleveltemp2, S1, S2, CDnewV, SM1, SM2);
						}
						else {
							SrVTemp[i][j] = interp2(sleveltemp1, sleveltemp2, S1, S2, CTnewV, SM1, SM2);	
							SrVTemp[i][j] -= interp2(sleveltemp1, sleveltemp2, S1, S2, CDnewV, SM1, SM2);
						}
					}
					else {
	...
	/////
				...
				if(m_matu[curtN-1] == 0 && m_ctime < 15) {		
					...
					for(intraLoop=intraTN; intraLoop>=19; intraLoop--) {  // here 19 확인		
						...
						//533333335333333333333333333333333333333333333333333333
						...
						//////////////////cd  
			///////////////3333333333333333333332222222222222222222222222222222
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					if(m_modeltype == 3) { // CT(x) 원금보장, CT(O): 원금비보장
						if(m_CTFlag == 1) 
							knocknewV[i][j] = AMOUNT *  (-1 + MIN(MIN(S1[i],S2[j]),1));
						else
							knocknewV[i][j] = AMOUNT * (m_Cpn[curtN+1]);
					}
...
void StepDownCTCDB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownCTCDB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownCTCDB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownCTCDB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownCTCDB::fd_pricei(double *greeks)
{
