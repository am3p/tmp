#include "StepDownCTB.h"
#include <iostream>
#include <fstream>

 

StepDownCTB::StepDownCTB(int kiFlag, int koFlag, int CTFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *CThval, double *Cpn,
...
{		
	...
	m_CTCpn[m_matuN] = Cpn[m_matuN+2 + m_matuN];
	m_CTCpn[m_matuN+1] = Cpn[m_matuN+2 + m_matuN+1];
	
	m_CTFlag = CTFlag;
	m_CTmatu = matu[m_matuN];
	m_CThval1 = CThval[0];
	m_CThval2 = CThval[1];
...
double StepDownCTB::getVega(int vegaN, int *vegaidx) {
...
double StepDownCTB::getRho(int rhoN,int *rhoidx) {
...
double StepDownCTB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownCTB::cf_price()
{	
	return 7777;
}

double StepDownCTB::fd_price(double *greeks)
{
	...
	if(m_CTmatu >= 0 && m_CTFlag == 0)  {  
		...
		double **NCTXMAu, **NCTXMAd, **NCTXMAl;
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dy <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				...
				while (temps <= m_Smaxlevel2 + 2*myeps) {
					...
					if(temps > CThlevel2 - Admesh + myeps ) //CT
						temph = AdmeshR;
					if(temps > CThlevel2 + Admesh + myeps)
						temph = dy;
		...
		CToldV = (double **)malloc((size_t)(SM1+1)*sizeof(double *));
		...
		for(i=0; i<SM1+1; i++) {
			...
			CToldV[i] = (double *)malloc((size_t)(SM2+1)*sizeof(double));	
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					CTknocknewV[i][j] = AMOUNT * (1+m_CTCpn[curtN-1]);
					...
					if(m_modeltype == 0 || m_modeltype == 10) { // 원금비보장 stepdown
						knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1);		
						CTknocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1);	
					}
					...
					if(i>hidx1 && j>hidx2) {
						newV[i][j] = AMOUNT * (1+m_Cpn[curtN]); // dummy cpn 저장
						CTnewV[i][j] = AMOUNT * (1+m_CTCpn[curtN]); // dummy cpn 저장
					}
					else {
						newV[i][j] = knocknewV[i][j];
						CTnewV[i][j] = CTknocknewV[i][j];
				...
				for(j=0; j<=SM2; j++) {
					if(i<=xidx1 || j<=xidx2) {
						...
						newV[i][j] = MAX(newV[i][j],spreadvalue);
							...
							//spreadvalue = AMOUNT * (1+m_CTCpn[curtN-1]-1)/(spmax-spmin) * (MIN(S1[i],S2[j])-spmin)+1;
			...
			for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 
				...
				for(i=1; i<SM1; i++) { //x축 풀기 위하여 행렬 만들기
					...
					if(SmeshType1 == 0) { //균등 mesh
						...
						knockXMAd[i][j] = v1*v1*i*i*dt + 1 +  0.5*rd*dt; 
			...
			for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 
				...
				for(j=1; j<SM2; j++) { //x축 풀기 위하여 행렬 만들기
					...
					if(SmeshType2 == 0) { //균등 mesh
						...
						knockYMAd[i][j] = v2*v2*j*j*dt + 1 + 0.5*rd*dt;
			...
			for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
					...
					for(j=1; j<SM2; j++) { //y축 고정 풀기
						...
						if(j<endSidx2) { // 행사 안되는 영역  if(j<endSidx2)
							...
							for(i=endSidx1; i<=SM1; i++)
								tU[i][j] = AMOUNT * (1+m_CTCpn[curtN-1]);
					...
					for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 				 
						if(j<endSidx2) {
							...
							for(i=1; i<endSidx1; i++)
								mytempV[i][j] = vecV1[i] - knockXMAl[i][j]*mytempV[i-1][j];
							for(i=endSidx1; i<=SM1; i++)
								mytempV[i][j] = AMOUNT * (1+m_CTCpn[curtN-1]);
							for(i=endSidx1-1; i>=0; i--) 
								mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
						}				
					} // for(j y축.. 고정 다 품
					...
					if(m_spreadXRate[curtN-1] > 0){// && m_CTCpn[curtN-1]>0 ) { // X spread는 만기와 조기상환일에 가능
						...
						spvalueR =  AMOUNT * (1+m_CTCpn[curtN-1]);
						for(i=xidx1; i<=SM1; i++) {
							maxindexR = 0;
						...
						for(i=startSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
							if(i<endSidx1) {
								...
								for(j=startSidx2+1; j<endSidx2; j++) 
									CTnewV[i][j] = vecV2[j] - YMAl[i][j]*CTnewV[i][j-1];					
								for(j=endSidx2; j<=SM2; j++)
									CTnewV[i][j] = AMOUNT * (1+m_CTCpn[curtN-1]);
				...
				if(matuLoop > m_CTmatu) {
					...
					*/
					...
					if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
						...
						for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 				 
							if(j<endSidx2) {
								...
								for(i=1; i<endSidx1; i++)
									mytempV[i][j] = vecV1[i] - knockXMAl[i][j]*mytempV[i-1][j];
								for(i=endSidx1; i<=SM1; i++)
									mytempV[i][j] = AMOUNT * (1+m_Cpn[curtN-1]);
								for(i=endSidx1-1; i>=0; i--) 
									mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
							}				
						} // for(j y축.. 고정 다 품
						...
						for(i=0; i<=SM1; i++) 
							for(j=0; j<=SM2; j++)
								knocknewV[i][j] = (mytempV[i][j] + knocknewV[i][j])/2.0;
						...
						*/
						...
						if(m_kiFlag != 1) {		
							...
							for(j=startSidx2; j<=SM2; j++)
								tU[startSidx1][j] = knocknewV[startSidx2][j];
	...
	*/	
			...
			using std::ofstream;	 
			ofstream logsgnuplot;	 
			logsgnuplot.open("c:/logland/kzveriCTB88.txt",ios::out);	 

			logsgnuplot << "newV[85][85] " << newV[85][85] << endl;
			logsgnuplot << "newV[84][84] " << newV[84][84] << endl;
			logsgnuplot << "CTnewV[85][85] " << CTnewV[85][85] << endl;
			logsgnuplot << "CTnewV[84][84] " << CTnewV[84][84] << endl;

			logsgnuplot << "newV[87][87] " << newV[87][87] << endl;
			logsgnuplot << "newV[88][88] " << newV[88][88] << endl;
			logsgnuplot << "CTnewV[87][87] " << CTnewV[87][87] << endl;
			logsgnuplot << "CTnewV[88][88] " << CTnewV[88][88] << endl;
			logsgnuplot << "m_CTmatu " << m_CTmatu << endl;
...
void StepDownCTB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownCTB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownCTB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownCTB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
