...
StepDownCPCDC::StepDownCPCDC(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
double StepDownCPCDC::getDeltaGammaTheta() {
...
double StepDownCPCDC::getVega(int vegaN, int *vegaidx) {
...
double StepDownCPCDC::getRho(int rhoN,int *rhoidx) {
...
double StepDownCPCDC::mc_price()
{
	...
	for(si=0; si<m_noiters; si++) { //MC SIM
		...
		for(sk=0; sk<m_matuN; sk++) {
			...
			if(CntAs == AsNUM3) {
				simprice = simprice + AMOUNT * (m_Cpn[sk])*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
			...
			if(sk==m_matuN-1) {
				if(simkiFlag == 0)
					simprice = simprice + AMOUNT *(m_Cpn[sk+1])*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				else {
					Wps = simsval[0]/m_bval[0];
					for(k=1; k<AsNUM3; k++) 
						Wps = MIN(Wps,simsval[k]/m_bval[k]);
					simprice = simprice + AMOUNT * (-1 + MIN(Wps,1)) * exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				}
...
void StepDownCPCDC::curttimeVol(double *vol, int day, int Asidx)
{
	int i;
...
int StepDownCPCDC::curttimeCorridx(int day)
{
	int i;		
	int findidx;
...
void StepDownCPCDC::curttimeFXVol(double *vol, int day, int Asidx)
{
	int i;
...
double StepDownCPCDC::curttimeFXCorr(int day, int Asidx)
{
	int i;		
	int findidx;
...
/*
double StepDownCPCDC::curttimeFXCorr2(int day)
{
	int i;		
	int findidx;
...
int StepDownCPCDC::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
