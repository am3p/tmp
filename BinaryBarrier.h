...
class BinaryBarrier : public Basic
{
public:
	BinaryBarrier(int duFlag, int ioFlag, int cpFlag, int knockFlag,
		  double sval, double bval, double xval, double hval, double Cpn,
...
private:
	...
	int m_BinaryBarrierCode; // m_BinaryBarrierCode = 6*max(m_duFlag,0) + 3*max(m_ioFlag,0) + 1*(m_cpFlag+1) 으로
	  /*    11	13	    'DIDC'
			10	5	    'DIF'
			9	17	    'DIDP'
			8	21	    'DODC'
