...
BinaryBarrier::BinaryBarrier(int duFlag, int ioFlag, int cpFlag, int knockFlag, double sval, double bval, double xval, double hval, 
	      double Cpn,
...
{		
	...
	m_BinaryBarrierCode = 6*MAX(m_duFlag,0) + 3*MAX(m_ioFlag,0) + 1*(m_cpFlag+1);
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/debug_binarybarrierinput.txt",ios::out);
...
double BinaryBarrier::getVega(int vegaN, int *vegaidx) {
...
double BinaryBarrier::getRho(int rhoN,int *rhoidx) {
...
double BinaryBarrier::cf_price()
{	
	return 7777;
}

double BinaryBarrier::fd_price(double *greeks)
{
...
void BinaryBarrier::curttimeVol(double *vol, int day)
{
	int i;
