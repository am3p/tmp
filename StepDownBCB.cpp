#include "StepDownBCB.h"
#include <iostream>
#include <fstream>

 

StepDownBCB::StepDownBCB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
{		
	...
	m_bcxval1 = new double[m_matuN]; //bc
	m_bcxval2 = new double[m_matuN];
	...
	for(i=0; i<m_matuN; i++) {
		m_xval1[i] = xval[i];
		m_xval2[i] = xval[2*m_matuN+i];
		m_Cpn[i] = Cpn[i];
		m_matu[i] = matu[i];
		m_spreadXRate[i] = abs(spreadXRate[i]);				

		m_bcxval1[i] = xval[m_matuN+i]; // bc
		m_bcxval2[i] = xval[3*matuN+i];
		m_bcCpn[i] = Cpn[m_matuN+2 +i];
	}
...
double StepDownBCB::getVega(int vegaN, int *vegaidx) {
...
double StepDownBCB::getRho(int rhoN,int *rhoidx) {
...
double StepDownBCB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownBCB::cf_price()
{	
	return 7777;
}

double StepDownBCB::fd_price(double *greeks)
{
	 
	if(m_modeltype == 2)  {  // kostart
		double DCValue;
		double DCMaxValue;
		...
		double kohlevel1, kohlevel2; //koadd
		...
		int hidx1, hidx2;
		int kohidx1, kohidx2; //koadd
		int evali;
		int evalq, expiryq;

		int bcxidx1, bcxidx2; //bc
		...
		kohlevel1 = m_kohval1/m_bval1; // koadd
		bcxlevel1 = m_bcxval1[curtN-1]/m_bval1; //bc
		...
		kohlevel2 = m_kohval2/m_bval2; // koadd
		bcxlevel2 = m_bcxval2[curtN-1]/m_bval2; // bc
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				double *tempgenS;
				tempgenS = new double[m_meshSM1 + int(6*Admesh/AdmeshR)+6]; //koadd !!
...
*/		 
		...
		if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dy <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
			...
			if(m_meshSM3 == 1) {
				...
				while (temps <= m_Smaxlevel2 + 2*myeps) {
					...
					if(temps > meshxlevel2 + Admesh + myeps)
						temph = dy;
					if(temps > kohlevel2 - Admesh + myeps) // koadd
						temph = AdmeshR;
					if(temps > kohlevel2 + Admesh + myeps)
						temph = dy;
		...
		bcxidx1 = findinteridx(bcxlevel1, S1, SM1); //bc
		...
		bcxidx2 = findinteridx(bcxlevel2, S2, SM2); // bc
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					//모형별 만기 payoff
					if(m_modeltype == 2) { // 원금비보장 stepdown  // koadd
						knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1);	  // AMOUNT * MIN(S1[i],S2[j]);			
					}
				...
				if(i>=bcxidx1 && j>=bcxidx2) {
					knocknewV[i][j] += AMOUNT * (m_bcCpn[curtN-1]); //bc
					newV[i][j] = knocknewV[i][j];
				}
			}
		}
		...
		*/
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			if(mid_n > 0) { //
				...
				bcxlevel1 = m_bcxval1[curtN-1]/m_bval1;
				bcxlevel2 = m_bcxval2[curtN-1]/m_bval2;
				...
				bcxidx1 = findinteridx(bcxlevel1, S1, SM1);	
				bcxidx2 = findinteridx(bcxlevel2, S2, SM2);	
				...
				*/
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
					...
					for(j=1; j<SM2; j++) { //y축 고정 풀기
						...
						if(j<kohidx2) { // 행사 안되는 영역  if(j<endSidx2)
							for(i=1; i<SM1; i++) 
					...
					for(j=1; j<SM2; j++) { //y축 고정 풀기
						...
						if(j<endSidx2) { // 행사 안되는 영역  if(j<endSidx2)
							...
							if(j<bcxidx2) {
								...
								for(i=endSidx1; i<=SM1; i++)
									tU[i][j] = AMOUNT * (1+m_Cpn[curtN-1]);
								for(i=bcxidx1; i<=SM1; i++)
									tU[i][j] += AMOUNT * (m_bcCpn[curtN-1]);
								for(i=endSidx1-1; i>=0; i--)
									tU[i][j] = (tU[i][j] - knockXMAu[i][j]*tU[i+1][j])/knockXMAd[i][j];
							}
						}
						...
						if(j<endSidx2) {
							...
							if(j<bcxidx2) {
								...
								for(i=endSidx1; i<=SM1; i++)
									mytempV[i][j] = AMOUNT * (1+m_Cpn[curtN-1]);
								for(i=bcxidx1; i<=SM1; i++)
									mytempV[i][j] += AMOUNT * (m_bcCpn[curtN-1]);
								for(i=endSidx1-1; i>=0; i--) 
									mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
							}
						}				
							...
							if(i<endSidx1) {
								...
								if(i<bcxidx1) {
									...
									for(j=endSidx2; j<=SM2; j++)
										newV[i][j] = AMOUNT * (1+m_Cpn[curtN-1]);
									for(j=bcxidx2; j<=SM2; j++)
										newV[i][j] += AMOUNT * (m_bcCpn[curtN-1]);
									for(j=endSidx2-1; j>=startSidx2; j--) 
										newV[i][j] = (newV[i][j] - YMAu[i][j]*newV[i][j+1])/YMAd[i][j];					
								}
							}				
					...
					DCMaxValue = knocknewV[SM1][SM2];  //koadd
	...
	/////
				...
				if(m_matu[curtN-1] == 0 && m_ctime < 15) {		
					...
					for(intraLoop=intraTN; intraLoop>=19; intraLoop--) {  // here 19 확인		
						DCMaxValue = DCMaxValue * 1/(1+rd/(YFactor*intraTN));
						DCValue = DCValue * 1/(1+rd/(YFactor*intraTN));
		...
		for(i=0; i<=SM1; i++) {
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					if(m_modeltype == 201) { // 원금보장 KI 있는 stepdown
						if(i>hidx1 && j>hidx2)						
							knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1);
						else
							knocknewV[i][j] = AMOUNT * (1+m_Cpn[curtN+1]);
...
void StepDownBCB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownBCB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownBCB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownBCB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownBCB::fd_price0(double *greeks)
{
	double DCValue;
	int i, j, matuLoop, intraLoop;
	int SM1, SM2, SmeshType1, SmeshType2, intraTN;
		...
		for(j=0; j<=SM2; j++) {
			if(i>=xidx1 && j>=xidx2) {
				...
				if(m_modeltype == 0) { // 원금비보장 stepdown
					knocknewV[i][j] = AMOUNT * MIN(S1[i],S2[j]);			
				}
	...
	*/
	...
	for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
		...
		for(i=1; i<SM1; i++) { //x축 풀기 위하여 행렬 만들기
			...
			if(SmeshType1 == 0) { //균등 mesh
				knockXMAl[i] = -0.5*(v1*v1)*(i*i)*dt + (r-q1)*i*dt/2.0 ;
				knockXMAd[i] = v1*v1*i*i*dt + 0.5*rd*dt + 1;
				knockXMAu[i] = -0.5*(v1*v1)*(i*i)*dt - (r-q1)*i*dt/2.0 ;				 
			}
			else {						
				knockXMAl[i] = -(v1*v1)*(S1[i]*S1[i])*dt/(h1[i-1]*(h1[i]+h1[i-1])) + (r-q1)*S1[i]*dt/(h1[i]+h1[i-1]) ;
				knockXMAd[i] = (v1*v1)*(S1[i]*S1[i])*dt/(h1[i-1]*h1[i]) + 0.5*rd*dt + 1;
				knockXMAu[i] = -(v1*v1)*(S1[i]*S1[i])*dt/(h1[i]*(h1[i]+h1[i-1])) - (r-q1)*S1[i]*dt/(h1[i]+h1[i-1])  ;						 
			}				
		...
		//경계 선형 조건
		if(SmeshType1 == 0) {
			knockXMAl[SM1-1] = knockXMAl[SM1-1] - knockXMAu[SM1-1];
			knockXMAd[SM1-1] = knockXMAd[SM1-1] + 2*knockXMAu[SM1-1];
		}
		else {			 
			knockXMAl[SM1-1] = knockXMAl[SM1-1] - knockXMAu[SM1-1]*h1[SM1-1]/h1[SM1-2];
			knockXMAd[SM1-1] = knockXMAd[SM1-1] + knockXMAu[SM1-1]*(h1[SM1-1]+h1[SM1-2])/h1[SM1-2];			
		}

		if(m_kiFlag != 1) {
			XMAl[SM1-1] = knockXMAl[SM1-1];			
			XMAd[SM1-1] = knockXMAd[SM1-1];
			...
			//LU decomposition			
			for(i=startSidx1+1; i<=SM1-1; i++) {				
				XMAl[i] = XMAl[i]/XMAd[i-1];				
				XMAd[i] = XMAd[i] - XMAl[i]*XMAu[i-1];			
			}			
		}

		//LU decomposition
		for(i=1; i<=SM1-1; i++) {
			knockXMAl[i] = knockXMAl[i]/knockXMAd[i-1];
			knockXMAd[i] = knockXMAd[i] - knockXMAl[i]*knockXMAu[i-1];
		}				
		  //X 행렬 끝
		...
		for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 
			...
			for(i=1; i<SM1; i++) { //x축 풀기 위하여 행렬 만들기
				...
				if(SmeshType1 == 0) { //균등 mesh					 
					coeV1[i][j] = 0.5*cor*v1*v2*i*j*dt/4.0;
				}
				else {							
					coeV1[i][j] = 0.5*cor*v1*v2*S1[i]*S2[j]*dt/((h1[i]+h1[i-1])*(h2[j]+h2[j-1]));	
				}				
			} // 행렬 만들기		
		}
		...
		for(j=1; j<SM2; j++) { //x축 풀기 위하여 행렬 만들기
			...
			if(SmeshType2 == 0) { //균등 mesh
				knockYMAl[j] = -0.5*(v2*v2)*(j*j)*dt + (r-q2)*j*dt/2.0 ;
				knockYMAd[j] = v2*v2*j*j*dt + 0.5*rd*dt + 1;
				knockYMAu[j] = -0.5*(v2*v2)*(j*j)*dt - (r-q2)*j*dt/2.0  ;
 
			}
			else {						
				knockYMAl[j] = -(v2*v2)*(S2[j]*S2[j])*dt/(h2[j-1]*(h2[j]+h2[j-1])) + (r-q2)*S2[j]*dt/(h2[j]+h2[j-1])  ;
				knockYMAd[j] = (v2*v2)*(S2[j]*S2[j])*dt/(h2[j-1]*h2[j]) + 0.5*rd*dt + 1;
				knockYMAu[j] = -(v2*v2)*(S2[j]*S2[j])*dt/(h2[j]*(h2[j]+h2[j-1])) - (r-q2)*S2[j]*dt/(h2[j]+h2[j-1])  ;							
		...
		//경계 선형 조건
		if(SmeshType2 == 0) {
			knockYMAl[SM2-1] = knockYMAl[SM2-1] - knockYMAu[SM2-1];
			knockYMAd[SM2-1] = knockYMAd[SM2-1] + 2*knockYMAu[SM2-1];
		}
		else {			 
			knockYMAl[SM2-1] = knockYMAl[SM2-1] - knockYMAu[SM2-1]*h2[SM2-1]/h2[SM2-2];
			knockYMAd[SM2-1] = knockYMAd[SM2-1] + knockYMAu[SM2-1]*(h2[SM2-1]+h2[SM2-2])/h2[SM2-2];			
		}

		if(m_kiFlag != 1) {
			YMAl[SM2-1] = knockYMAl[SM2-1];			
			YMAd[SM2-1] = knockYMAd[SM2-1];
			...
			//LU decomposition			
			for(j=startSidx2+1; j<=SM2-1; j++) {				
				YMAl[j] = YMAl[j]/YMAd[j-1];				
				YMAd[j] = YMAd[j] - YMAl[j]*YMAu[j-1];			
			}			
		}

		//LU decomposition
		for(j=1; j<=SM2-1; j++) {
			knockYMAl[j] = knockYMAl[j]/knockYMAd[j-1];									
			knockYMAd[j] = knockYMAd[j] - knockYMAl[j]*knockYMAu[j-1];
		}				
		...
		for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 
			...
			for(j=1; j<SM2; j++) { //x축 풀기 위하여 행렬 만들기
				...
				if(SmeshType2 == 0) { //균등 mesh
					...
					coeV2[i][j] = 0.5*cor*v1*v2*i*j*dt/4.0;
				}
				else {						
				 
					coeV2[i][j] = 0.5*cor*v1*v2*S1[i]*S2[j]*dt/((h2[j]+h2[j-1])*(h1[i]+h1[i-1]));	
				}				 
			}
			...
			*/
			...
			if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
				...
				for(j=1; j<SM2; j++) { //y축 고정 풀기
					...
					for(i=1; i<SM1; i++) 
						vecV1[i] = knockoldV[i][j] + coeV1[i][j]*(knockoldV[i+1][j+1]+knockoldV[i-1][j-1]-knockoldV[i-1][j+1]-knockoldV[i+1][j-1]);

					tU[0][j] = DCValue; //0.0;
					for(i=1; i<SM1; i++)
						tU[i][j] = vecV1[i] - knockXMAl[i]*tU[i-1][j];
					tU[SM1-1][j] = tU[SM1-1][j]/knockXMAd[SM1-1];
					for(i=SM1-2; i>=0; i--) 				
						tU[i][j] = (tU[i][j] - knockXMAu[i]*tU[i+1][j])/knockXMAd[i];
				...
				for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 				 
					...
					for(j=1; j<SM2; j++) 				
						vecV2[j] =tU[i][j] + coeV2[i][j] *(tU[i+1][j+1]+tU[i-1][j-1]-tU[i-1][j+1]-tU[i+1][j-1]);					   				 		 		

					knocknewV[i][0] = DCValue; //0.0;
					for(j=1; j<SM2; j++) 
						knocknewV[i][j] = vecV2[j] - knockYMAl[j]*knocknewV[i][j-1];					
					knocknewV[i][SM2-1] = knocknewV[i][SM2-1]/knockYMAd[SM2-1];
					for(j=SM2-2; j>=0; j--) 
						knocknewV[i][j] = (knocknewV[i][j] - knockYMAu[j]*knocknewV[i][j+1])/knockYMAd[j];					
				...
				if(m_kiFlag != 1) {				
					...
					for(j=startSidx2+1; j<SM2; j++) { //y축 고정 풀기						
						for(i=startSidx1+1; i<SM1; i++) 
							vecV1[i] = oldV[i][j] + coeV1[i][j]*(oldV[i+1][j+1]+oldV[i-1][j-1]-oldV[i-1][j+1]-oldV[i+1][j-1]);
                        
						tU[startSidx1][j] = knocknewV[startSidx1][j];
						for(i=startSidx1+1; i<SM1; i++)
							tU[i][j] = vecV1[i] - XMAl[i]*tU[i-1][j];
						tU[SM1-1][j] = tU[SM1-1][j]/XMAd[SM1-1];
						for(i=SM1-2; i>=startSidx1; i--) 				
							tU[i][j] = (tU[i][j] - XMAu[i]*tU[i+1][j])/XMAd[i];
					...
					for(i=startSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
						...
						newV[i][startSidx2] = knocknewV[i][startSidx2];
						for(j=startSidx2+1; j<SM2; j++) 
							newV[i][j] = vecV2[j] - YMAl[j]*newV[i][j-1];					
						newV[i][SM2-1] = newV[i][SM2-1]/YMAd[SM2-1];
						for(j=SM2-2; j>=startSidx2; j--) 
							newV[i][j] = (newV[i][j] - YMAu[j]*newV[i][j+1])/YMAd[j];					
				...
				for(j=1; j<SM2; j++) { //y축 고정 풀기
					if(j<endSidx2 || j >= endSidx2) { // 행사 안되는 영역
						for(i=1; i<SM1; i++) 
							vecV1[i] = knockoldV[i][j] + coeV1[i][j]*(knockoldV[i+1][j+1]+knockoldV[i-1][j-1]-knockoldV[i-1][j+1]-knockoldV[i+1][j-1]);
...
double StepDownBCB::fd_price1(double *greeks)
{
