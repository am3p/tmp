...
StepDownC::StepDownC(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
double StepDownC::getDeltaGammaTheta() {
...
double StepDownC::getVega(int vegaN, int *vegaidx) {
...
double StepDownC::getRho(int rhoN,int *rhoidx) {
	...
	for(i=0; i<m_corrN; i++) {
		for(cok=0; cok<AsNUM3; cok++) {
			...
			if(0) {
				...
				logsgnuplot.open("c:/logland/SDCveri.txt",ios::out);	 
	 
				for(coi=0; coi<AsNUM3; coi++)  {
					for(coj=0; coj<AsNUM3; coj++) 
						logsgnuplot << Cholcor[0][coi][coj] << "   ";
					logsgnuplot <<  endl;
				}
	...
	for(si=0; si<m_noiters; si++) { //MC SIM
		...
		for(sk=0; sk<m_matuN; sk++) {
			...
			if(l==100 && si == 3) {
				...
				using std::ofstream;	 
				ofstream logsgnuplot;	 
				logsgnuplot.open("c:/logland/SDCveri3.txt",ios::out);	 
	  
				
				for(int cooi=0; cooi<AsNUM3; cooi++)  {
					for(int cooj=0; cooj<2; cooj++) 
						logsgnuplot << m_FXcorr[cooi][cooj] << "   ";
					logsgnuplot <<  endl;
			...
			*/
			...
			if(sk==m_matuN-1) {
				if(simkiFlag == 0)
					...
					simprice = AMOUNT * MIN(Wps,1)*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				}
...
void StepDownC::curttimeVol(double *vol, int day, int Asidx)
{
	int i;
...
int StepDownC::curttimeCorridx(int day)
{
	int i;		
	int findidx;
...
void StepDownC::curttimeFXVol(double *vol, int day, int Asidx)
{
	int i;
...
double StepDownC::curttimeFXCorr(int day, int Asidx)
{
	int i;		
	int findidx;
...
/*
double StepDownC::curttimeFXCorr2(int day)
{
	int i;		
	int findidx;
...
int StepDownC::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
