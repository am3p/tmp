...
StepDownCPC::StepDownCPC(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
{		
	...
	m_payxval = new double*[AsNUM3];
	for(i=0; i<AsNUM3; i++)
		m_payxval[i] = new double[m_paymatuN];

	for(i=0; i<AsNUM3; i++) {
		for(j=0; j<m_paymatuN; j++) {
			m_payxval[i][j] = xval[m_paymatuN*i + j];
		}
	}
	...
	m_matuN = matuN[1];  // cd는 m_matuN , m_CPN 순서로 m_matuN이 [0]이지만 pay는 m_paymatuN, m_matuN 순서라 [1] 주의!!
	...
	for(i=0; i<AsNUM3; i++) {
		for(j=0; j<m_matuN; j++) {
			m_xval[i][j] = xval[m_paymatuN*AsNUM3 + m_matuN*i+j];
		}
	}
...
double StepDownCPC::getDeltaGammaTheta() {
...
double StepDownCPC::getVega(int vegaN, int *vegaidx) {
...
double StepDownCPC::getRho(int rhoN,int *rhoidx) {
		...
		for(sk=0; sk<m_matuN; sk++) {
			...
			for(;l<=m_matu[sk]; l++) {
				...
				for(k=0; k<m_paymatuN; k++) {
					if(l == m_paymatu[k]) {
						int Ask;
						for(Ask=0; Ask<AsNUM3; Ask++) {
							if(simsval[Ask] < m_payxval[Ask][k]) {
								CntAs--;
								break;
							}
						}
						...
						if(CntAs == AsNUM3) {
							simprice = simprice + AMOUNT * (temppayCpn[k]) * exp(-dailyZdc[m_paymatu[k]]*m_paymatu[k]/YFactor);
						}
						break;
					}
				}
			...
			if(CntAs == AsNUM3) {
				simprice = simprice + AMOUNT * (1+m_Cpn[sk])*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				break;
			}

			if(sk==m_matuN-1) {
				if(simkiFlag == 0)
					simprice = simprice + AMOUNT *(1+m_Cpn[sk+1])*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				else {
					Wps = simsval[0]/m_bval[0];
					for(k=1; k<AsNUM3; k++) 
						Wps = MIN(Wps,simsval[k]/m_bval[k]);
					simprice = simprice + AMOUNT * MIN(Wps,1)*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				}
...
void StepDownCPC::curttimeVol(double *vol, int day, int Asidx)
{
	int i;
...
int StepDownCPC::curttimeCorridx(int day)
{
	int i;		
	int findidx;
...
void StepDownCPC::curttimeFXVol(double *vol, int day, int Asidx)
{
	int i;
...
double StepDownCPC::curttimeFXCorr(int day, int Asidx)
{
	int i;		
	int findidx;
...
/*
double StepDownCPC::curttimeFXCorr2(int day)
{
	int i;		
	int findidx;
...
int StepDownCPC::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
