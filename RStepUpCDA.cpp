#include "RStepUpCDA.h"
#include <iostream>
#include <fstream>

 

RStepUpCDA::RStepUpCDA(int kiFlag, double sval, double bval, double *xval, double kihval, 
	      double *Cpn,
...
{		
	...
	m_spreadHS = m_kihval + m_bval*m_spreadHRate; // up

	/*
	for(i=0; i<m_matuN; i++) 	  
		m_spreadXS[i] = m_xval[i] + m_bval*m_spreadXRate[i]; // up
		*/
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/rstepupcdainput.txt",ios::out);
...
double RStepUpCDA::getVega(int vegaN, int *vegaidx) {
...
double RStepUpCDA::getRho(int rhoN,int *rhoidx) {
...
double RStepUpCDA::cf_price()
{	
	return 7777;
}

double RStepUpCDA::fd_price(double *greeks)
{
	...
	for(i=0; i<=SM; i++) {
		if(i<=xidx) {
			...
			if(m_modeltype == 0 || m_modeltype == 10) 			
				knocknewV[i] = AMOUNT * MAX(MIN(0,1-S[i]),-1); //(-1.0 + MIN(S[i],1));   //S[i];			AMOUNT * (-1.0 + S[i]);  //AMOUNT * MAX(MIN(0,xlevel-S[i]),-1);
		...
		for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
			...
			if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
				...
				if(m_kiFlag != 1) {
					...
					// L*tempV = oldV forward로 풀기
					newV[0] = DCValue; // <- oldV에 복사했으므로 꼭 필요한 작업은 아님
					for(i=1; i<=startSidx-1; i++)
						newV[i] = oldV[i] - knockMAl[i]*newV[i-1];

					//U*newV = tempV backward로 풀기
					for(i=startSidx; i<=SM; i++)						
						newV[i] = knocknewV[i];

					for(i=startSidx-1; i>=1; i--)
						newV[i] = (newV[i] - knockMAu[i]*newV[i+1])/knockMAd[i];
				...
				for(i=0; i<=endSidx; i++)					
					knocknewV[i] =  AMOUNT * (m_Cpn[curtN-1]); //0.0; // D 0 조건

				//for(i=2; i<=SM-1; i++) // Smin N 조건
				for(i=endSidx+1; i<SM; i++) // Smin D 조건
					knocknewV[i] = knockoldV[i] - MAl[i]*knocknewV[i-1];

				knocknewV[SM-1] = knocknewV[SM-1]/MAd[SM-1];
				//U*newV = tempV backward로 풀기
				for(i=SM-2; i>=endSidx; i--)
					knocknewV[i] = (knocknewV[i] -MAu[i]*knocknewV[i+1])/MAd[i];
...
/*
	//kzR here
				...
				for(i=0; i<=endSidx; i++)					
					CDnewV[i] =   AMOUNT * (CpnCDleg[curtN-1]); //0.0; // D 0 조건

				//for(i=2; i<=SM-1; i++) // Smin N 조건
				for(i=endSidx+1; i<SM; i++) // Smin D 조건
					CDnewV[i] = CDoldV[i] - MAl[i]*CDnewV[i-1];

				CDnewV[SM-1] = CDnewV[SM-1]/MAd[SM-1];
				//U*newV = tempV backward로 풀기
				for(i=SM-2; i>=endSidx; i--)
					CDnewV[i] = (CDnewV[i] -MAu[i]*CDnewV[i+1])/MAd[i];
				...
				if(m_spreadXRate[curtN-1] > 0){// && CpnCDleg[curtN-1]>0) { // X spread는 만기와 조기상환일에 가능
					...
					for(i=xidx; i<=SM; i++) { 
						//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(xlevel - spreadXSlevel) * (S[i]-spreadXSlevel) + 0);
						//spreadvalue = AMOUNT * (((1+CpnCDleg[curtN-1])-1)/(S[xidx] - S[spreadXSidx]) * (S[i]-S[spreadXSidx]) + 1);
						spreadvalue = AMOUNT * ( -1.0*m_spreadXRate[curtN-1] * (S[i]-S[xidx]) + 1+CpnCDleg[curtN-1]);
						//knocknewV[i] = MAX(knocknewV[i],spreadvalue);	
	...
	//kzR here
				...
				*/
				...
				if(m_kiFlag != 1) {
					...
					//startS, endS는 확정 조건
					for(i=0; i<=endSidx; i++)						
						newV[i] = AMOUNT *  (m_Cpn[curtN-1]);

					for(i=endSidx+1; i<=startSidx-1; i++)
...
void RStepUpCDA::curttimeVol(double *vol, int day)
{
	int i;
