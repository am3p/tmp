...
class StepDownCDA : public Basic
{
public:
	//int CDN, double CDadjval, int *ObBDay, int *PayBDay, double *FlegFactor, double *ObCDrate, // cd
	StepDownCDA(int kiFlag, int koFlag, double sval, double bval, double *xval, double kihval, double kohval, double *Cpn,
...
private:
	...
	int m_CDN;
	double m_CDadjval;
	int *m_ObBDay; // AccX 이상 쌓인 날수(관찰된) 공용
	int *m_PayBDay; // 관찰기간 공용
	double *m_FlegFactor;  // 1/4 * AR 공용
	double *m_ObCDrate; // 9에서는 사용 안함
