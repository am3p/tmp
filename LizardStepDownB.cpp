#include "LizardStepDownB.h"
#include <iostream>
#include <fstream>

extern odysseyRandom *opr;

LizardStepDownB::LizardStepDownB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
LizardStepDownB::~LizardStepDownB()
{	
	int i,k;	 
	delete m_evalN;
...
double LizardStepDownB::getValue() {
...
double LizardStepDownB::getDeltaGammaTheta() {
...
double LizardStepDownB::getVega(int vegaN, int *vegaidx) {
...
double LizardStepDownB::getRho(int rhoN,int *rhoidx) {
...
/*
double LizardStepDownB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double LizardStepDownB::mc_price()
{
...
*/	
	...
	for(si=0; si<m_noiters; si++) { //MC SIM
		...
		for(sk=0; sk<m_matuN; sk++) {
			...
			if(m_matu[sk] == m_lizardmatu && simkiFlag == 0) {
				simprice = AMOUNT * (1+m_Cpn[m_matuN])*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				break;
			}
			...
			if(sk==m_matuN-1) {
				if(simkiFlag == 0)
					...
					simprice = AMOUNT * (1+MIN((Wps-1),0))*exp(-dailyZdc[m_matu[sk]]*m_matu[sk]/YFactor);
				}
			}
			*/
...
void LizardStepDownB::curttimeVol(double *vol, int day, int Asidx)
{
	int i;
...
int LizardStepDownB::curttimeCorridx(int day)
{
	int i;		
	int findidx;
