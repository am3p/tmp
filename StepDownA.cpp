...
StepDownA::StepDownA(int kiFlag, int koFlag, double sval, double bval, double *xval, double kihval, double kohval,
	      double *Cpn,
...
{		
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/stepdownainput.txt",ios::out);
...
double StepDownA::getVega(int vegaN, int *vegaidx) {
...
double StepDownA::getRho(int rhoN,int *rhoidx) {
...
double StepDownA::cf_price()
{	
	return 7777;
}

double StepDownA::fd_price(double *greeks)
{
	if(m_modeltype == 2) { // koadd
		double DCValue;
		double DCMaxValue;
		...
		kohlevel = m_kohval/m_bval; // koadd
		...
		*/
		...
		for(i=0; i<=SM; i++) {
			if(i>=xidx) {
				...
				if(m_modeltype == 0 || m_modeltype == 2) 			
					knocknewV[i] = AMOUNT * MIN(1,S[i])*(1+m_stax); //AMOUNT * S[i];			
		...
		if(m_spreadHRate > 0) { // KI 쪽 spread 만기에만 
			...
			for(i=spreadHSidx; i<=hidx; i++) {
				...
				if(hidx== spreadHSidx)
					spreadvalue = AMOUNT * (((1+m_Cpn[curtN])-0)/(S[hidx] - S[spreadHSidx-1]) * (S[i]-S[spreadHSidx-1]) + 0);
				else
		...
		*/
		...
		if(m_spreadXRate[curtN-1] > 0) { // X spread는 만기와 조기상환일에 가능
			...
			for(i=0; i<=xidx; i++) {
				...
				//if(xidx==spreadXSidx)				
					//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-1)/(S[xidx] - S[spreadXSidx-1]) * (S[i]-S[spreadXSidx-1]) + 1);
				//else
					//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-1)/(S[xidx] - S[spreadXSidx]) * (S[i]-S[spreadXSidx]) + 1);
		...
		for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
			...
			for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
				...
				if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
					...
					for(i=kohidx; i<=SM; i++)				
						knocknewV[i] =  DCMaxValue; // AMOUNT * (1+m_Cpn[curtN-1]); // D 행사 조건으로 처리

					for(i=kohidx-1; i>=0; i--)
					...
					*/
					...
					if(m_kiFlag != 1) {
						...
						//U*newV = tempV backward로 풀기
						for(i=kohidx; i<=SM; i++) 
							newV[i] = DCMaxValue; //AMOUNT * (1+m_Cpn[curtN-1]); // D 행사 조건으로 처리
		...
		if(m_curtgreek == 1 || m_curtgreek == 2) {
			for(i=0; i<outN + 4; i++) {			
				...
				if(sim_knockKOFlag == 1)			
					SrVTemp[i] = DCMaxValue; // (0: flat, 1: 선형)
			}
	...
	/////
		...
		*/
		...
		if(m_modeltype == 25) {
			for(i=0; i<=SM; i++) {
				if(i>=xidx) 
					knocknewV[i] = AMOUNT * (1+MAX(S[i]-xlevel,0)*m_Cpn[curtN-1]);  // booster m_Cpn[matuN-1]에 참여율 저장!!
				else {
					if(i>=hidx)
						knocknewV[i] = AMOUNT * 1;
					else
						knocknewV[i] = AMOUNT * S[i];
				}
			}
		} // 25: booster
		else {
...
void StepDownA::curttimeVol(double *vol, int day)
{
	int i;
