#include "StepDownCDB.h"
#include <iostream>
#include <fstream>

 

StepDownCDB::StepDownCDB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
double StepDownCDB::getVega(int vegaN, int *vegaidx) {
...
double StepDownCDB::getRho(int rhoN,int *rhoidx) {
...
double StepDownCDB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownCDB::cf_price()
{	
	return 7777;
}

double StepDownCDB::fd_price(double *greeks)
{
	...
	if(m_modeltype == 9) { // DA
			...
			if(m_sval1 <= m_kihval1 || m_sval2 <= m_kihval2)			
				...
				double DCValue, CDlegDCValue, AccDCValue;
				...
				double kohlevel1, kohlevel2; // ko add
				...
				kohlevel1 = m_kohval1/m_bval1; // ko
				...
				kohlevel2 = m_kohval2/m_bval2; // ko
				...
				if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
					...
					if(m_meshSM3 == 1) {
						double *tempgenS;
						tempgenS = new double[m_meshSM1 + int(6*Admesh/AdmeshR)+6]; // ko
				...
				kohidx1 = findinteridx(kohlevel1, S1, SM1); // ko
				...
				kohidx2 = findinteridx(kohlevel2, S2, SM2); // ko
				...
				for(i=0; i<=SM1; i++) {
					for(j=0; j<=SM2; j++) {
						if(i>=xidx1 && j>=xidx2) {
							...
							knocknewV[i][j] = AMOUNT *  (-1 + MIN(MIN(S1[i],S2[j]),1)*(1+m_stax)); //MIN(S1[i],S2[j]);	//cd    AMOUNT *  (-1 + MIN(S1[i],S2[j]));
						...
						CDnewV[i][j] = 0;//AMOUNT * CpnCDleg[curtN-1]; // CD 는 전구간 동일
					}

					AccDCValue = AMOUNT * m_FlegFactor[curtN-1]/m_PayBDay[curtN-1];
				}
				...
				*/
					...
					for(i=0; i<outN + 4; i++) {
						for(j=0; j<outN + 4; j++) {
							...
							if(m_matu[curtN-1] == 0) {
								...
								SrV1Temp[i][j] += m_ObBDay[curtN-1] * AccDCValue;
								...
								if(i>=kohidx1 && j>=kohidx2) 
									SrV1Temp[i][j] += (m_ObBDay[curtN-1] +1) * AccDCValue;
								else
									SrV1Temp[i][j] += m_ObBDay[curtN-1] * AccDCValue;
				...
				for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						...
						if(intraLoop == intraTN) {
							 
							for(i=0; i<=SM1; i++) {
								for(j=0; j<=SM2; j++) {								
									if(i>=kohidx1 && j>=kohidx2)									
										CDnewV[i][j] += AccDCValue;
								}
							}
						}
							...
							for(j=1; j<SM2; j++) { //y축 고정 풀기
								if(j<endSidx2) { // 행사 안되는 영역
									...
									for(i=1; i<endSidx1; i++) 
										tU[i][j] = vecV1[i] - knockXMAl[i][j]*tU[i-1][j];
									for(i=endSidx1; i<=SM1; i++)
										tU[i][j] = AMOUNT * 0;  //(1+m_Cpn[curtN-1]);   //cd
									for(i=endSidx1-1; i>=0; i--)
										tU[i][j] = (tU[i][j] - knockXMAu[i][j]*tU[i+1][j])/knockXMAd[i][j];
								}
							}// for(j=1
							...
							for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 				 
								if(i<endSidx1) {
									...
									for(j=1; j<endSidx2; j++)
										CDnewV[i][j] = vecV2[j] - knockYMAl[i][j]*CDnewV[i][j-1];
									for(j=endSidx2; j<=SM2; j++)
										CDnewV[i][j] = AMOUNT * 0;//(1+m_Cpn[curtN-1]); //cd
									for(j=endSidx2-1; j>=0; j--) 
										CDnewV[i][j] = (CDnewV[i][j] - knockYMAu[i][j]*CDnewV[i][j+1])/knockYMAd[i][j];
								}				
							} // for(j y축.. 고정 다 품
							...
							for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 				 
								if(j<endSidx2) {
									...
									for(i=1; i<endSidx1; i++)
										mytempV[i][j] = vecV1[i] - knockXMAl[i][j]*mytempV[i-1][j];
									for(i=endSidx1; i<=SM1; i++)
										mytempV[i][j] = AMOUNT * 0;//(1+m_Cpn[curtN-1]); //cd
									for(i=endSidx1-1; i>=0; i--) 
										mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
								}				
							} // for(j y축.. 고정 다 품
								...
								for(i=spreadXSidx1; i<=SM1; i++) {
									for(j=spreadXSidx2; j<=SM2; j++) {
										if(i<=xidx1 || j<=xidx2) {
											...
											//knocknewV[i][j] = MAX(knocknewV[i][j], spreadvalue);	
											spreadvalue = AMOUNT * (1+0)/(spmax-spmin) * (MIN(S1[i],S2[j])-spmin);
					...
					AccDCValue = AccDCValue * 1/(1+rd/(YFactor));
				...
				if(debugFlag) {
					...
					for(i=0; i<=SM1; i++) {						
						for(j=0; j<=SM2; j++) {
							...
							valuetemp += m_ObBDay[curtN-1] * AccDCValue;
				...
				if(m_curtgreek == 1 || m_curtgreek == 2) {
					for(i=0; i<outN + 4; i++) {			
						for(j=0; j<outN + 4; j++) {
							...
							if(m_ctime > 15 && slevel1 >= xlevel1 && slevel2 >= xlevel2 && m_matu[curtN-1] == 0)
								SrVTemp[i][j] += m_ObBDay[curtN-1] * AMOUNT * m_FlegFactor[curtN-1]/m_PayBDay[curtN-1];
							else
								SrVTemp[i][j] += m_ObBDay[curtN-1] * AccDCValue;
				...
				if(m_curtgreek == 1) { // 기본 그릭  
					...
					if(m_ctime > 15 && slevel1 >= xlevel1 && slevel2 >= xlevel2 && m_matu[curtN-1] == 0)
						value += m_ObBDay[curtN-1] * AMOUNT * m_FlegFactor[curtN-1]/m_PayBDay[curtN-1];
					else				
						value += m_ObBDay[curtN-1] * AccDCValue;
	...
	/////////////////////////////////////////////////////// DA
		...
		if(m_modeltype == 32 || m_modeltype == 33) { // 32: Digital option , 33: One Touch
			...
			if(m_modeltype == 32) {			 
				...
				double DCValue, KODCValue;//, CDlegDCValue;	
				...
				//double spreadHSlevel1, spreadHSlevel2, spreadXSlevel1, spreadXSlevel2;
				//double spreadXSlevel1, spreadXSlevel2;
				double *S1, *h1, *S2, *h2;
				double **oldV, **newV, **tU;
				double **knockoldV, **knocknewV;	
				...
				//double startSlevel1, startSlevel2, endSlevel1, endSlevel2;
				double endSlevel1, endSlevel2;

				int xidx1, xidx2;
				//int startSidx1, startSidx2, endSidx1, endSidx2;
				int endSidx1, endSidx2;
				//int spreadHSidx1, spreadHSidx2, spreadXSidx1, spreadXSidx2;
				//int spreadXSidx1, spreadXSidx2;
				//int hidx1, hidx2;
				int evali;
				int evalq, expiryq;
				...
				*/
				...
				//D.O의 만기 payoff는.. 왼쪽... 이라서..
				//m_spreadXS1[m_matuN-1] = 2*m_xval1[m_matuN-1] - m_spreadXS1[m_matuN-1];
				//m_spreadXS2[m_matuN-1] = 2*m_xval2[m_matuN-1] - m_spreadXS2[m_matuN-1];
				...
				if((m_meshSM2 <=0) || (m_meshSM3 <=0) || dx <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
					...
					if(m_meshSM3 == 1) {
						double *tempgenS;
						tempgenS = new double[m_meshSM1 + int(2*Admesh/AdmeshR)+2];
				...
				for(i=0; i<=SM1; i++) {
					for(j=0; j<=SM2; j++) {
						if(i<xidx1 || j<xidx2) {
							...
							knocknewV[i][j] = AMOUNT * 0.0; //(m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]); //cd
				...
				KODCValue = AMOUNT * (m_Cpn[curtN-1]);
				...
				*/
				...
				if(m_spreadXRate[curtN-1] > 0){// && m_Cpn[curtN-1]>0) { // X spread는 만기와 조기상환일에 가능
					...
					for(i=xidx1; i<=SM1; i++) { 
						for(j=xidx2; j<=SM2; j++) { 
								...
								spreadvalue = AMOUNT * ( -1.0*m_spreadXRate[curtN-1] * (MIN(S1[i],S2[j])-spmin) + 1+m_Cpn[curtN-1]);
				...
				for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						DCValue = DCValue * 1/(1+rd/(YFactor*intraTN));
						KODCValue = KODCValue * 1/(1+rd/(YFactor*intraTN));
						//CDlegDCValue = CDnewV[0][0] * 1/(1+rd/(YFactor*intraTN));
						//memcpy(knockoldV,knocknewV,(size_t)((SM+1)*sizeof(double))); //만기 payoff를 newV에 저장했으므로..
						for(i=0; i<=SM1; i++)			
							memcpy(knockoldV[i],knocknewV[i],(size_t)((SM2+1)*sizeof(double)));
						...
						*/
						...
						if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)				
							...
							*/
							...
							for(j=1; j<SM2; j++) { //y축 고정 풀기
								if(j<endSidx2) { // 행사 안되는 영역
									...
									for(i=1; i<endSidx1; i++) 
										tU[i][j] = vecV1[i] - knockXMAl[i][j]*tU[i-1][j];
									for(i=endSidx1; i<=SM1; i++)
										tU[i][j] = KODCValue;//AMOUNT * (m_Cpn[curtN-1]);  //(1+m_Cpn[curtN-1]); //cd
									for(i=endSidx1-1; i>=0; i--)
										tU[i][j] = (tU[i][j] - knockXMAu[i][j]*tU[i+1][j])/knockXMAd[i][j];
								}
							}// for(j=1
							...
							for(i=1; i<SM1; i++) {  //y축 고정시켜서.. 				 
								if(i<endSidx1) {
									...
									for(j=1; j<endSidx2; j++)
										knocknewV[i][j] = vecV2[j] - knockYMAl[i][j]*knocknewV[i][j-1];
									for(j=endSidx2; j<=SM2; j++)
										knocknewV[i][j] = KODCValue;//AMOUNT * (m_Cpn[curtN-1]);   //(1+m_Cpn[curtN-1]); //cd
									for(j=endSidx2-1; j>=0; j--) 
										knocknewV[i][j] = (knocknewV[i][j] - knockYMAu[i][j]*knocknewV[i][j+1])/knockYMAd[i][j];
								}				
							} // for(j y축.. 고정 다 품
							...
							for(j=1; j<SM2; j++) {  //y축 고정시켜서.. 				 
								if(j<endSidx2) {
									...
									for(i=1; i<endSidx1; i++)
										mytempV[i][j] = vecV1[i] - knockXMAl[i][j]*mytempV[i-1][j];
									for(i=endSidx1; i<=SM1; i++)
										mytempV[i][j] = KODCValue;//AMOUNT * (m_Cpn[curtN-1]);   //(1+m_Cpn[curtN-1]); //cd
									for(i=endSidx1-1; i>=0; i--) 
										mytempV[i][j] = (mytempV[i][j] - knockXMAu[i][j]*mytempV[i+1][j])/knockXMAd[i][j];
								}				
							} // for(j y축.. 고정 다 품
							...
							if(m_spreadXRate[curtN-1] > 0){// && KODCValue>0) { // X spread는 만기와 조기상환일에 가능
								double spreadvalue;
								double   spmax;
						...
						spreadvalue = AMOUNT * ( m_spreadXRate[curtN-1] * (MIN(S1[i],S2[j])-spmax) + 1+KODCValue);

											//spreadvalue = (1+KODCValue-1)/(spmax-spmin) * (MIN(S1[i],S2[j])-spmin) +1;//AMOUNT * (1+m_Cpn[curtN-1])/(spmax-spmin) * (MIN(S1[i],S2[j])-spmin);
											//knocknewV[i][j] = MAX(knocknewV[i][j], spreadvalue);	//cd
											knocknewV[i][j] = MAX(knocknewV[i][j], spreadvalue-AMOUNT);
	...
	//			double IFRtemp;
		...
		m_spreadHS1 = 2*m_kihval1 - m_spreadHS1;		
		m_spreadHS2 = 2*m_kihval1 - m_spreadHS2;
		...
		m_startS2 = m_spreadHS2;
				...
				for(i=0; i<=SM1; i++) {
					for(j=0; j<=SM2; j++) {
						...
						if(i>=xidx1 && j>=xidx2) {						 
							...
							if(i>hidx1 && j>hidx2) 
								newV[i][j] = 0.0;//AMOUNT * (m_Cpn[curtN]);  //(1+m_Cpn[curtN]); // dummy cpn 저장
							else
								newV[i][j] = knocknewV[i][j];
						}
				...
				if(m_spreadHRate > 0) { // KI 쪽 spread 만기에만 
					...
					for(i=hidx1; i<=SM1; i++) {
						for(j=hidx2; j<=SM2; j++) {
							if(i<=spreadHSidx1 || j<=spreadHSidx2) {
								if(S1[i]<=S2[j]) {
									spmin = S1[hidx1];
									spmax = S1[spreadHSidx1];
								}
								else {
									spmin = S2[hidx2];
									spmax = S2[spreadHSidx2];
								}
								...
								for(i=startSidx1+1; i<SM1; i++) {  //y축 고정시켜서.. 				 
									if(i<endSidx1) {
										...
										for(j=startSidx2+1; j<endSidx2; j++) 
											newV[i][j] = vecV2[j] - YMAl[i][j]*newV[i][j-1];					
										for(j=endSidx2; j<=SM2; j++)
											newV[i][j] = KODCValue;//AAMOUNT *  (m_Cpn[curtN-1]);  //(1+m_Cpn[curtN-1]);   //cd
										for(j=endSidx2-1; j>=startSidx2; j--) 
											newV[i][j] = (newV[i][j] - YMAu[i][j]*newV[i][j+1])/YMAd[i][j];					
									}				
								} // for(j y축.. 고정 다 품
								...
								for(i=startSidx1+1; i<SM1; i++) { //y축 고정 풀기
									if(i<endSidx1) {
										...
										for(j=startSidx2+1; j<endSidx2; j++) 
											tU[i][j] = vecV2[j] - YMAl[i][j]*tU[i][j-1];
										for(j=endSidx2; j<=SM2; j++)
											tU[i][j] = KODCValue;//AAMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]); //cd
										for(j=endSidx2-1; j>=startSidx2; j--)
											tU[i][j] = (tU[i][j] - YMAu[i][j]*tU[i][j+1])/YMAd[i][j];
									}
								}// for(j=1
								...
								for(j=startSidx2+1; j<SM2; j++) {  //y축 고정시켜서.. 				 
									if(j<endSidx2) {
										...
										for(i=startSidx1+1; i<endSidx1; i++) 
											mytempV[i][j] = vecV1[i] - XMAl[i][j]*mytempV[i-1][j];					
										for(i=endSidx1; i<=SM1; i++)
											mytempV[i][j] = KODCValue;//AAMOUNT *  (m_Cpn[curtN-1]);  //(1+m_Cpn[curtN-1]);   //cd
										for(i=endSidx1-1; i>=startSidx1; i--) 
											mytempV[i][j] = (mytempV[i][j] - XMAu[i][j]*mytempV[i+1][j])/XMAd[i][j];					
									}				
								} // for(j y축.. 고정 다 품
			...
			*/	
			...
			if(m_modeltype == 2) {
				...
				*/
				...
				CDlegDCMaxValue = CDnewV[SM1][SM2];
				...
				for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						...
						CDlegDCMaxValue = CDnewV[SM1][SM2] * 1/(1+rd/(YFactor*intraTN));
						...
						*/
						...
						spreadvalue = AMOUNT * ( m_spreadXRate[curtN-1] * (MIN(S1[i],S2[j])-spmax) + 1+m_Cpn[curtN-1]);
							...
							CDlegDCMaxValue = CDnewV[SM1][SM2];  //koadd
						...
						if(i>=xidx1 && j>=xidx2) {
							...
							if(m_modeltype == 4 || m_modeltype == 14) {
								if(i>hidx1 && j>hidx2)
									...
									knocknewV[i][j] = AMOUNT * (-1 + MIN(MIN(S1[i]/hlevel1,S2[i]/hlevel2),1));
							}
...
void StepDownCDB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownCDB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownCDB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownCDB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownCDB::fd_pricei(double *greeks)
{
		...
		for(j=0; j<=SM2; j++) {
			if(i>=xidx1 && j>=xidx2) {
				...
				if(m_modeltype == 0) { // 원금비보장 stepdown
					knocknewV[i][j] = AMOUNT *  (-1 + MIN(MIN(S1[i],S2[j]),1)*(1+m_stax)); //MIN(S1[i],S2[j]);	//cd
				}
