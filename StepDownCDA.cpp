#include "StepDownCDA.h"
#include <iostream>
#include <fstream>

 

StepDownCDA::StepDownCDA(int kiFlag, int koFlag, double sval, double bval, double *xval, double kihval, double kohval,
	      double *Cpn,
...
double StepDownCDA::getVega(int vegaN, int *vegaidx) {
...
double StepDownCDA::getRho(int rhoN,int *rhoidx) {
...
double StepDownCDA::cf_price()
{	
	return 7777;
}

double StepDownCDA::fd_price(double *greeks)
{
	if(m_modeltype == 32 || m_modeltype == 33) {
		if(m_modeltype == 32) { //D.O				 
			...
			m_kiFlag = 1;
				...
				double slevel, xlevel;// , spreadXSlevel;
				double *S, *h;
				double *oldV, *newV;
				...
				//m_spreadXS[m_matuN-1] = 2*m_xval[m_matuN-1] - m_spreadXS[m_matuN-1];
				...
				*/
				...
				KODCValue = AMOUNT * (m_Cpn[curtN-1]);
					for(i=0; i<=SM; i++) {
						if(i<xidx) {
							...
							knocknewV[i] = AMOUNT * 0.0;   //S[i];			AMOUNT * (-1.0 + S[i]);
				...
				if(m_spreadXRate[curtN-1] > 0){// && m_Cpn[curtN-1]>0) { // X spread는 만기와 조기상환일에 가능
					...
					for(i=xidx; i<=SM; i++) { 
						...
						//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(S[xidx] - S[spreadXSidx]) * (S[i]-S[spreadXSidx]) + 0);
						spreadvalue = AMOUNT * (-1.0*m_spreadXRate[curtN-1] * (S[i]-S[xidx]) + 1+m_Cpn[curtN-1]);
						//knocknewV[i] = MAX(knocknewV[i],spreadvalue);
						//newV[i] = MAX(newV[i],spreadvalue);
						knocknewV[i] = MAX(knocknewV[i],spreadvalue-AMOUNT); // 주의!! 위로 올려서 스프레드 적용 -> 다시 내림!
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						...
						if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
							...
							for(i=endSidx; i<=SM; i++)				
								knocknewV[i] =  KODCValue;//AMOUNT * (m_Cpn[curtN-1]);// (1+m_Cpn[curtN-1]); // D 행사 조건으로 처리
							...
							*/
							...
							if(m_spreadXRate[curtN-1] > 0){// && KODCValue>0) { // X spread는 만기와 조기상환일에 가능
								...
								for(i=0; i<=xidx; i++) {
									//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(xlevel - spreadXSlevel) * (S[i]-spreadXSlevel) + 0);
									//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(S[xidx] - S[spreadXSidx]) * (S[i]-S[spreadXSidx]) + 0);
									spreadvalue = AMOUNT * (m_spreadXRate[curtN-1] * (S[i]-S[xidx]) + 1+KODCValue);
		...
		else {	//m_modeltype == 33	// One Touch D.O
							 
			
	m_spreadHS = 2*m_kihval - m_spreadHS;		
	...
	if(m_sval <= m_kihval)			
		m_kiFlag = 1;
				...
				*/
				...
				KODCValue = AMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]);
					for(i=0; i<=SM; i++) {
						knocknewV[i] = AMOUNT * (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]);
						if(i>=xidx) {							 
							newV[i] = knocknewV[i];
							...
							if(i>hidx) 
								newV[i] = AMOUNT * 0.0;//(m_Cpn[curtN]);   //(1+m_Cpn[curtN]); // dummy cpn 저장
							else
								newV[i] = knocknewV[i];
						}
				...
				if(m_spreadHRate > 0) { // KI 쪽 spread 만기에만 
					double spreadvalue;
						...
						for(i=hidx; i<=spreadHSidx; i++) { 
							...
							spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(S[spreadHSidx] - S[hidx]) * (S[i]-S[hidx]) + 0);
							//newV[i] = MAX(newV[i],spreadvalue);
						...
						if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
							...
							*/
							...
							if(m_kiFlag != 1) {
								...
								//U*newV = tempV backward로 풀기
								for(i=endSidx; i<=SM; i++) 
									newV[i] = KODCValue;//AMOUNT *  (m_Cpn[curtN-1]); //(1+m_Cpn[curtN-1]); // D 행사 조건으로 처리
	...
	if(m_sval >= m_kohval)
		...
		if(m_modeltype == 9) {		 
				...
				double DCValue, CDlegDCValue, AccDCValue;//, AccDCValuetemp;	
				...
				if((m_meshSM2 <=0) || (m_meshSM3 <=0) || ds <= AdmeshR ) { // || (xidx1 - m_meshSM2 < 0) || (m_meshSM1 < xlevel1 + m_meshSM2)) { 
					...
					if(m_meshSM3 == 1) {
						...
						while (temps <= m_Smaxlevel + 2*myeps) {
							...
							if(temps > hlevel + Admesh + myeps)
								temph = ds;
							if(temps > kohlevel - Admesh + myeps )
								temph = AdmeshR;
							if(temps > kohlevel + Admesh + myeps)
								temph = ds;
							if(temps > meshxlevel - Admesh + myeps)
				...
				*/
				...
				for(i=0; i<SM; i++) {
					if(S[i] < kohlevel && kohlevel <= S[i+1]) {
						kohidx = i+1;
						break;
					}
				}
				...
				expiryq = 1;
					...
					for(i=0; i<=SM; i++) {
						if(i>=xidx) {
							...
							knocknewV[i] = AMOUNT * (-1.0 + MIN(S[i],1)*(1+m_stax));   //S[i];			AMOUNT * (-1.0 + S[i]);
				...
				*/
				...
				if(m_curtgreek == 1 && (m_matu[curtN-1] == 1 || m_matu[curtN-1] == 0)) { // 기본 그릭	Theta를 위한 +1일 저장
					for(i=0; i<outN + 4; i++) {												
						...
						if(m_matu[curtN-1] == 0) {
							...
							SrV1Temp[i] += m_ObBDay[curtN-1] * AccDCValue;
						}
						else {						
							if(sim_knockFlag == 1) {
								SrV1Temp[i] = interp1(sleveltemp,S,knocknewV,SM+1,SmeshType,1); // (0: flat, 1: 선형)
							...
							if(i>=kohidx)
								SrV1Temp[i] += (m_ObBDay[curtN-1] +1) * AccDCValue;
							else
								SrV1Temp[i] += m_ObBDay[curtN-1] * AccDCValue;
						}
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						...
						if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
							...
							*/
							...
							//U*newV = tempV backward로 풀기
							for(i=endSidx; i<=SM; i++)				
								CDnewV[i] =  0;  //(1+m_Cpn[curtN-1]); // D 행사 조건으로 처리
						...
						//AccDCValue = AccDCValue * 1/(1+rd/(YFactor*intraTN));
					}//for(intraLoop)
				...
				if(m_curtgreek == 1 || m_curtgreek == 2) {
					for(i=0; i<outN + 4; i++) {			
						...
						if(m_ctime > 15 && slevel >= xlevel && m_matu[curtN-1] == 0)
							SrVTemp[i] += m_ObBDay[curtN-1] * AMOUNT * m_FlegFactor[curtN-1]/m_PayBDay[curtN-1];
						else
							SrVTemp[i] += m_ObBDay[curtN-1] * AccDCValue;
			...
			*/		
			...
			if(m_modeltype == 2) {
					...
					if(m_matu[i] >= 0) {
						for(j=0; j<m_CDN-1; j++) {
							if(m_matu[i]<=m_PayBDay[j]) {
								...
								if(m_PayBDay[j] == m_matu[i]) {
									CpnCDleg[i] = FlegCpn[j];
								}
								else {
									if(m_FlegFactor[j] == 0)
										CpnCDleg[i] = 0.0;
				...
				*/
				...
				DCMaxValue = knocknewV[SM];
				CDlegDCMaxValue = CDnewV[SM];

				if(m_koFlag == 1) {
					for(i=0; i<=SM; i++) {
						knocknewV[i] = DCMaxValue;
						newV[i] = DCMaxValue;
						CDnewV[i] = CDlegDCMaxValue;
					}
				}
				...
				if(m_curtgreek == 1 && (m_matu[curtN-1] == 1 || m_matu[curtN-1] == 0)) { // 기본 그릭	Theta를 위한 +1일 저장
					for(i=0; i<outN + 4; i++) {												
						...
						if(sim_knockKOFlag == 1) {
							SrV1Temp[i] = DCMaxValue - CDlegDCMaxValue;
						}
					}		 
				}
				...
				for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
					...
					for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
						...
						CDlegDCValue = CDnewV[0] * 1/(1+rd/(YFactor*intraTN));
						CDlegDCMaxValue = CDnewV[SM] * 1/(1+rd/(YFactor*intraTN));
						...
						if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
							...
							knocknewV[0] =  DCValue; //0.0; // D 0 조건
							//for(i=2; i<=SM-1; i++) // Smin N 조건
							for(i=1; i<kohidx; i++) // Smin D 조건
								knocknewV[i] = knockoldV[i] - knockMAl[i]*knocknewV[i-1];

					 
							//U*newV = tempV backward로 풀기
							for(i=kohidx; i<=SM; i++)					
								knocknewV[i] = DCMaxValue;//knocknewV[SM-1]/knockMAd[SM-1];

							for(i=kohidx-1; i>=1; i--)
								knocknewV[i] = (knocknewV[i] - knockMAu[i]*knocknewV[i+1])/knockMAd[i];
							...
							*/
							...
							//U*newV = tempV backward로 풀기
							for(i=kohidx; i<=SM; i++)					
								CDnewV[i] = CDlegDCMaxValue; //CDnewV[SM-1]/knockMAd[SM-1];
							...
							*/
							...
							if(m_kiFlag != 1) {
								...
								// L*tempV = oldV forward로 풀기
								newV[startSidx] = oldV[startSidx]; // <- oldV에 복사했으므로 꼭 필요한 작업은 아님
								for(i=startSidx+1; i<kohidx; i++)
									newV[i] = oldV[i] - MAl[i]*newV[i-1];

								//U*newV = tempV backward로 풀기
								for(i=kohidx; i<=SM; i++)						
									newV[i] = DCMaxValue;//newV[SM-1]/MAd[SM-1];

								for(i=kohidx-1; i>=startSidx; i--)
									newV[i] = (newV[i] - MAu[i]*newV[i+1])/MAd[i];
				...
				if(m_curtgreek == 1 || m_curtgreek == 2) {
					for(i=0; i<outN + 4; i++) {			
						...
						if(sim_knockKOFlag == 1) {
							SrVTemp[i] = DCMaxValue - CDlegDCMaxValue;
						}
					}
			...
			/////
				...
				*/
				...
				if(m_modeltype == 25) {
					for(i=0; i<=SM; i++) {
						if(i>=xidx)
							knocknewV[i] = AMOUNT * (MAX(S[i]-xlevel,0)*m_Cpn[curtN-1]);  // booster m_Cpn[matuN-1]에 참여율 저장!!
						else {
							if(i>=hidx)
								knocknewV[i] = AMOUNT * 0;
							else
								knocknewV[i] = AMOUNT * (-1+S[i]);
						}
					}
				} // 25: booster
				else {
					for(i=0; i<=SM; i++) {
						if(i>=xidx) {
							...
							if(m_modeltype == 4 || m_modeltype == 14) { //super
								if(i>hidx) {
									knocknewV[i] = AMOUNT * (m_Cpn[curtN+1]);
								}
								else {
									knocknewV[i] = AMOUNT * (-1.0 + MIN(1,S[i]/hlevel));
								}
							}
				...
				if(m_spreadHRate > 0) { // KI 쪽 spread 만기에만 
					...
					if(m_modeltype == 25) {								
						for(i=spreadHSidx; i<=hidx; i++) {
							//spreadvalue = AMOUNT * (((1+m_Cpn[curtN])-0)/(hlevel - spreadHSlevel) * (S[i]-spreadHSlevel) + 0);
							spreadvalue = AMOUNT * ((1-0)/(S[hidx] - S[spreadHSidx]) * (S[i]-S[spreadHSidx]) + 0);				
							knocknewV[i] = MAX(knocknewV[i],spreadvalue-AMOUNT);
						}
...
void StepDownCDA::curttimeVol(double *vol, int day)
{
	int i;
