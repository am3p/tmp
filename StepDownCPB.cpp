#include "StepDownCPB.h"
#include <iostream>
#include <fstream>

 

StepDownCPB::StepDownCPB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
double StepDownCPB::getVega(int vegaN, int *vegaidx) {
...
double StepDownCPB::getRho(int rhoN,int *rhoidx) {
...
double StepDownCPB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownCPB::cf_price()
{	
	return 7777;
}

double StepDownCPB::fd_price(double *greeks)
{
	 
	if(m_modeltype == 121)  {  // 만기 KI 상품!
				
		m_kiFlag = 1; // 만기 KI 상품
		
		double DCValue;
		double DCMaxValue;  // 즉시 지급이라 할인 없음	
		...
		*/
		...
		DCMaxValue = AMOUNT * (1 + m_payCpn[m_paymatuN-1]);
		if(m_koFlag == 1) {
			for(i=0; i<=SM1; i++) 
				for(j=0; j<=SM2; j++) {
					newV[i][j] = DCMaxValue;
				...
				if(intraLoop == 1) {
					for(k=0; k<m_paymatuN; k++)
						if(matuLoop == m_paymatu[k]) {
							DCMaxValue = AMOUNT * (1 + m_payCpn[k]);
...
void StepDownCPB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownCPB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownCPB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownCPB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownCPB::fd_price0(double *greeks)
{
	double DCValue;
...
double StepDownCPB::fd_price1(double *greeks)
{
