...
StepDownB::StepDownB(int kiFlag, int koFlag, double *sval, double *bval, double *xval, double *kihval, double *kohval, double *Cpn,
...
double StepDownB::getVega(int vegaN, int *vegaidx) {
...
double StepDownB::getRho(int rhoN,int *rhoidx) {
...
double StepDownB::getCorrDelta(int corrdeltaN,int *corrdeltaidx) {
...
double StepDownB::cf_price()
{	
	return 7777;
}

double StepDownB::fd_price(double *greeks)
{
...
*/		 
			...
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					if(m_modeltype == 2) { // 원금비보장 stepdown  // koadd
						knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1)*(1+m_stax);	  // AMOUNT * MIN(S1[i],S2[j]);			
					}
	...
	*/	
	...
	/////
		...
		if(m_modeltype == 10 || m_modeltype == 11  || m_modeltype == 14)
			m_kiFlag = 1;
			...
			for(j=0; j<=SM2; j++) {
				if(i>=xidx1 && j>=xidx2) {
					...
					if(m_modeltype == 0 || m_modeltype == 10) { // 원금비보장 stepdown
						knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i],S2[j]),1)*(1+m_stax);			
					}
					...
					/* 1S
					if(m_modeltype == 4 || m_modeltype == 14) { //super
						if(i>hidx) {
					...
					*/
					...
					if(m_modeltype == 4 || m_modeltype == 14) {
						if(i>hidx1 && j>hidx2)
							knocknewV[i][j] = AMOUNT * (1+m_Cpn[curtN+1]);
						else
							knocknewV[i][j] = AMOUNT * MIN(MIN(S1[i]/hlevel1,S2[i]/hlevel2),1);
					}
...
void StepDownB::curttimeVol1(double *vol, int day)
{
	int i;
...
void StepDownB::curttimeVol2(double *vol, int day)
{
	int i;
...
double StepDownB::curttimeCorr(int day)
{
	int i;		
	int findidx;
...
int StepDownB::findinteridx(double x, double *Dx, int DxN)
{
	int i;		
	int findidx;
...
double StepDownB::fd_price0(double *greeks)
{
	double DCValue;
	int i, j, matuLoop, intraLoop;
	int SM1, SM2, SmeshType1, SmeshType2, intraTN;
		...
		for(j=0; j<=SM2; j++) {
			if(i>=xidx1 && j>=xidx2) {
				...
				if(m_modeltype == 0) { // 원금비보장 stepdown
					knocknewV[i][j] = AMOUNT * MIN(S1[i],S2[j])*(1+m_stax);			
				}
...
double StepDownB::fd_price1(double *greeks)
{
