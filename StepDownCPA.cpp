...
StepDownCPA::StepDownCPA(int kiFlag, int koFlag, double sval, double bval, double *xval, double kihval, double kohval, double *Cpn,
...
{		
	...
	*/
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/stepdowncpainput.txt",ios::out);
...
double StepDownCPA::getVega(int vegaN, int *vegaidx) {
...
double StepDownCPA::getRho(int rhoN,int *rhoidx) {
...
double StepDownCPA::cf_price()
{	
	return 7777;
}

double StepDownCPA::fd_price(double *greeks)
{
		...
		for(i=0; i<=SM; i++) {
			if(i>=xidx) {
				...
				if(m_modeltype == 0 || m_modeltype == 2) 			
					knocknewV[i] = AMOUNT * MIN(1,S[i]); //AMOUNT * S[i];			
...
void StepDownCPA::curttimeVol(double *vol, int day)
{
	int i;
