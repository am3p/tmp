...
FRN::FRN(double ctime,	
		 ...
		 /* 
		 (int kiFlag, double sval, double bval, double *xval, double kihval, 
	      double *Cpn,
...
{		
	...
	if(debugFlag) {
		...
		ofstream logs;
		logs.open("c:/logland/frninput.txt",ios::out);
...
double FRN::getVega(int vegaN, int *vegaidx) {
...
double FRN::getRho(int rhoN,int *rhoidx) {
...
double FRN::fd_price(double *greeks)
{
	...
	*/
	...
	for(i=0; i<=SM; i++) {
		if(i>=xidx) {
			...
			if(i>hidx) 
				newV[i] = 0;   //(1+m_Cpn[curtN]); // dummy cpn 저장
			else
				newV[i] = 0;
		}

		CDnewV[i] = AMOUNT * (1+CpnCDleg[curtN-1]); // CD 는 전구간 동일
	}
	...
	*/
	...
	for(matuLoop=m_matu[curtN-1]-1; matuLoop>=0; matuLoop--) {
		...
		for(intraLoop=intraTN; intraLoop>=1; intraLoop--) {
			//DCValue = DCValue * 1/(1+rd/(252*intraTN));
			...
			if(IsMidCompu == 0) { //조기 상환일이 아닌 평시..형태로 풀기 (조기상환일에 상환 안된 경우도 해당)
				...
				*/
				...
				if(m_spreadXRate[curtN-1] > 0) { // X spread는 만기와 조기상환일에 가능
					...
					for(i=spreadXSidx; i<=xidx; i++) {
						//spreadvalue = AMOUNT * (((1+m_Cpn[curtN-1])-0)/(xlevel - spreadXSlevel) * (S[i]-spreadXSlevel) + 0);
						spreadvalue = AMOUNT * (((1+CpnCDleg[curtN-1])-0)/(S[xidx] - S[spreadXSidx]) * (S[i]-S[spreadXSidx]) + 0);
						//knocknewV[i] = MAX(knocknewV[i],spreadvalue);	
	...
	if(debugFlag) {
		...
		logsgnuplot.open("c:/logland/debug_frngnufdprice.txt",ios::out);	 
		for(i=0; i<=SM; i++) {
			logsgnuplot << S[i]*m_bval <<"     "<< CDnewV[i] << endl;
		}		 
		logsgnuplot.close();

		logsgnuplot.open("c:/logland/debug_avefrngnufdprice.txt",ios::out);	 
...
void FRN::curttimeVol(double *vol, int day)
{
	int i;
